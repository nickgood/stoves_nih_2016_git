...
```{r}
  mass_carbon <-
    ...
    dplyr::group_by(id) %>%
    dplyr::filter(val >= 0, grepl("^co2$|^co$", pol)) %>% 
    tidyr::spread("pol", "val") %>%
    dplyr::summarise(mass_carbon = sum(val, na.rm = TRUE)) %>%
    dplyr::mutate(mass_carbon =  ifelse(grepl("^1A$|^2A$", id), NA, mass_carbon))  # ignore tests with missing fivegas data
```
...
```{r, echo=TRUE} 
  mass_fuel <- dplyr::left_join(mass_fuel, mass_carbon, by = "id") %>%

...
* 29A and 29B - no repeat
* 16B - no repeat
* 2B - repeated test twice (2D & 2E)
* 1C - repeated test (1D)
* 8C - repeated test (8D)
...
```{r carbonyls_bg_timeseries, echo=FALSE, fig.height = 16, fig.width = 16}
 bg %>%
    dplyr::filter(!is.na(conc)) %>%
...
```{r carbonyls_samples_timeseries, echo=FALSE, fig.height = 16, fig.width = 16}
 carbs_samples %>%
    dplyr::filter(!is.na(conc)) %>%
    dplyr::left_join(dplyr::select(samples, id, date), by = 'id') %>%
    ggplot(aes_string(x = "date", y = "conc")) +
...
carbonyl concentrations (ug per meter cubed)

* samples are background corrected

```{r carbonyls_stovetype, echo=FALSE, fig.height = 50, fig.width = 16}
  carbs_samples_p <-
    carbs_samples %>%
...
```{r carbonyls_fueltype, echo=FALSE, fig.height = 50, fig.width = 16}
  carbs_samples_p %>%
...
* 1A seems to be a consistent outlier across pollutants
...
```{r}
  saveRDS(carbs_merged, file = "../../r_data/carbs_merged.RDS")
```

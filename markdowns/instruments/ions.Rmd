...
```{r ions_bg_timeseries, echo=FALSE, fig.height = 10, fig.width =12}
 bg %>%
    dplyr::filter(!is.na(conc)) %>%
...
```{r ions_samples_timeseries, echo=FALSE, fig.height = 10, fig.width = 12}
 ions_samples %>%
    dplyr::filter(!is.na(conc)) %>%
    dplyr::left_join(dplyr::select(samples, id, date), by = 'id') %>%
    ggplot(aes_string(x = "date", y = "conc")) +
...
```{r ions_test_bg_conc, echo=FALSE, fig.height = 10, fig.width = 12}
  ions_merged %>%
...
```{r ions_stovetype, echo=FALSE, fig.height = 30, fig.width = 16}
  ions_samples_p <-
    ions_samples %>%
...
```{r ions_fueltype, echo=FALSE, fig.height = 30, fig.width = 16}
  ions_samples_p %>%

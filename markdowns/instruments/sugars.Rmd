...
```{r}
  sugars <- readRDS("../../r_data/ions.RDS")
...
```{r sugars_bg_timeseries, echo=FALSE, fig.height = 10, fig.width =12}
 bg %>%
    dplyr::filter(!is.na(conc)) %>%
...
```{r}
  saveRDS(bg, file = "../../r_data/sugars_bg.RDS")
```
...
```{r sugars_samples_timeseries, echo=FALSE, fig.height = 10, fig.width = 12}
 sugars_samples %>%
    dplyr::filter(!is.na(conc)) %>%
    dplyr::left_join(dplyr::select(samples, id, date), by = 'id') %>%
    ggplot(aes_string(x = "date", y = "conc")) +
...
```{r sugars_test_bg_conc, echo=FALSE, fig.height = 10, fig.width = 12}
  sugars_merged %>%
...
```{r sugars_stovetype, echo=FALSE, fig.height = 30, fig.width = 16}
  sugars_samples_p <-
    sugars_samples %>%
...
```{r sugars_fueltype, echo=FALSE, fig.height = 30, fig.width = 16}
  sugars_samples_p %>%
...
```{r}
  saveRDS(sugars_merged, file = "../../r_data/sugars_merged.RDS")
```

...
```{r, echo=FALSE}
  p3 <- 
    ultrafines_p %>%
      ggplot(aes(x = stove, y = mean)) +
      geom_histogram(stat = "identity", fill = "royalblue3") +
...
```{r figure_1, echo=FALSE, fig.width=10, fig.height=12}
  p1 + plot_spacer() + p2 + plot_spacer() + p3 + plot_spacer() + plot_layout(nrow = 6,
                                                             heights = c(1, 0.10, 1, 0.10, 1, 0.10))
```

```{r, fig.width=3, fig.height=2, echo=FALSE}
  pm_mass_p %>%
    dplyr::filter(grepl("fossil", stovecat)) %>%
    ggplot(aes(x = stove, y = val)) +
    ...
    scale_y_continuous(limits = c(0, 20), breaks = c(0, 10, 20), position = "right") +
    geom_text(data = range_pm %>%
              dplyr::filter(grepl("fossil", stovecat)),
              aes(x = stove, y = val, label = sym), size = 10, angle = 90, nudge_x = -0.05) +
    theme_minimal() +
...
```{r, fig.width=4, fig.height=2.5, echo=FALSE, eval=FALSE}
  co_p %>%
    dplyr::filter(grepl("fossil", stovecat)) %>%
    ggplot(aes(x = stove, y = mean)) +
    geom_histogram(fill = "mediumslateblue", stat = "identity") +
    geom_text(data = range_co %>%
              dplyr::filter(grepl("fossil", stovecat)),
    ...
    theme(text = element_text(size = 20),
          ...
          legend.position = "none") +
    guides(fill = guide_legend(title = NULL)) +
    facet_grid(.~stovecat, scales = "free", switch = "y")
```
...
```{r, fig.width=4, fig.height=2, echo=FALSE, eval=FALSE}
  ultrafines_p %>%
    dplyr::filter(grepl("fossil", stovecat)) %>%
    ggplot(aes(x = stove, y = mean)) +
    geom_histogram(fill = "royalblue3", stat = "identity") +
    geom_text(data = range_ultrafines %>%
              dplyr::filter(grepl("fossil", stovecat)),
              aes(x = stove, y = val, label = sym), size = 10, angle = 90, nudge_x = -0.02) +
    theme_bw() +

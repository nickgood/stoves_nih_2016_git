...
```{r, echo=FALSE, eval=FALSE}
  p1 <-
    pm_mass_p  %>%
    dplyr::filter(grepl("wood", stovecat)) %>%
  ...
  p2 <-
    pm_mass_p  %>%
    dplyr::filter(grepl("pellet", stovecat)) %>%
    ggplot(aes(x = stove, y = val)) +
    geom_histogram(aes(fill = pol), stat = "identity") +
    geom_histogram(data = ec_oc_ions_p %>%
                   dplyr::filter(grepl("pellet", stovecat)),
  ...
  p3 <-
    pm_mass_p %>%
    dplyr::filter(grepl("charcoal", stovecat)) %>%
    ggplot(aes(x = stove, y = val)) +
      geom_histogram(aes(fill = pol), stat = "identity") +
      geom_histogram(data = ec_oc_ions_p %>%
                     dplyr::filter(grepl("charcoal", stovecat)),
                     aes(x = stove, y = val, fill = pol, order = pol), stat = "identity", width = 0.75) +
      scale_fill_manual(values = c("gray30", "#E69F00", "snow", "#999999")) +
      #annotate("text", x = 1, y = 400, label = "888", size = 5) +
      #annotate("text", x = 2, y = 400, label = "527", size = 5) +
  ...
  p4 <-
    pm_mass_p %>%
    dplyr::filter(grepl("kerosene", stovecat)) %>%
    ggplot(aes(x = stove, y = val)) +
      geom_histogram(aes(fill = pol), stat = "identity") +
      geom_histogram(data = ec_oc_ions_p %>%
                       dplyr::filter(grepl("kerosene", stovecat)),
      ...
      scale_fill_manual(values = c("gray30", "#E69F00", "snow", "#999999")) +
      #annotate("text", x = 1, y = 10, label = "23", size = 5) +
      geom_text(data = range_pm %>%
                  dplyr::filter(grepl("kerosene", stovecat)),
  ...
  p5 <-
    pm_mass_p %>%
    dplyr::filter(grepl("lpg", stovecat)) %>%
    ggplot(aes(x = stove, y = val)) +
      geom_histogram(aes(fill = pol), stat = "identity") +
      geom_histogram(data = ec_oc_ions_p %>%
                       dplyr::filter(grepl("lpg", stovecat)),
...
```{r, echo=FALSE, fig.width=16, fig.height=10, eval=FALSE}
...
```{r, echo=FALSE}
  p1 <-
    pm_mass_p %>%
      dplyr::mutate(pol_category = "PM2.5 composition") %>%
      ggplot(aes(x = stove, y = val)) +
      geom_histogram(aes(fill = pol), stat = "identity") +
      geom_histogram(data = ec_oc_ions_p %>%
                     dplyr::mutate(pol_category = "PM2.5 composition"),
                     aes(x = stove, y = val, fill = pol, order = pol), stat = "identity", width = 0.75) +
      scale_fill_manual(values = c("gray30", "#E69F00", "snow", "#999999"), name = expression(paste(PM[2.5], ' composition'))) +
      scale_y_continuous(limits = c(0, 1050), breaks = c(0, 250, 500, 750, 1000)) +
      annotate("text", x = 1, y = 1050, label = "1610", size = 5) +
      annotate("text", x = 2, y = 1050, label = "2119", size = 5) +
      geom_text(data = range_pm,
      ...
      theme(text = element_text(size = 20),
            ...
            axis.title.x = element_blank(),
            legend.position = "right") +
      facet_grid(.~pol_category, scales = "free") +
  ...
  p3 <- 
    ultrafines_p %>%
      ...
      geom_text(data = range_ultrafines,
        ...
        ylab("particles per MJ delivered")
```
...
```{r figure_1, echo=FALSE, fig.width=16, fig.height=12}
 (p1 + plot_spacer() + p2 + plot_spacer() + p3) + plot_layout(nrow = 3, heights = c(1.25, 1, 1))
```

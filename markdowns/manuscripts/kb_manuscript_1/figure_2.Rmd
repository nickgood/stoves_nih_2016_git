...
```{r, echo=FALSE}
  p2 <-
    vocs_p %>%
    dplyr::mutate(pol_category = "volatile organic compounds (VOCs)") %>%
    ggplot(aes(x = stove, y = val)) +
      geom_histogram(aes(fill = pol_subcategory, order = pol_subcategory), stat = "identity") +
...
```{r, echo=FALSE}
  p3 <-
    pahs_p %>%
    dplyr::mutate(pol_category = "polycyclic aromatic hydrocarbons (PAHs)") %>%
    ggplot(aes(x = stove, y = val)) +
      geom_histogram(aes(fill = pol_subcategory, order = pol_subcategory), stat = "identity") +
...
```{r figure_1b, echo=FALSE, fig.width=16, fig.height=12}

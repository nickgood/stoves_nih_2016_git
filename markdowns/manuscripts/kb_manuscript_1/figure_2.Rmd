...
```{r figure_2, echo=FALSE, fig.width=12, fig.height=8}
  p1 + p2 + plot_layout(nrow = 2, heights = c(1, 1))
```

```{r figure_2, echo=FALSE, fig.width=12, fig.height=4}
    pm_mass_p %>%
    dplyr::mutate(pol_category = "paste(PM[2.5], ' composition')") %>%
      ggplot(aes(x = stove, y = val)) +
      geom_histogram(fill = "#999999", stat = "identity") +
...
```{r, echo=FALSE, fig.width=12, fig.height=2}
    ...
    dplyr::mutate(pol_category = "paste(PM[2.5], ' composition')") %>%
      ...
      scale_fill_manual(values = c("#E69F00", "gray30", "snow2", "#999999"),
                        ...
                        drop = FALSE) +
      guides(fill = guide_legend(nrow = 2, byrow = FALSE)) +
      scale_y_continuous(limits = c(0, 15), breaks = c(0, 250, 500, 750, 1000)) +

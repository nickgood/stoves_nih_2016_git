...
```{r, echo=FALSE}
  p1 <-
    ...
    dplyr::mutate(pol_category = "paste(PM[2.5], ' composition')") %>%
      ...
      ylab(expression(atop(paste(PM[2.5], ' composition'), paste('grams per kilogra', m[fuel]))))
```
  ...
  p2 <-
    co_p %>%
      ...
      theme(text = element_text(size = 18),
            ...
            axis.title.x = element_blank(),
            legend.title = element_blank()) +
      ylab(expression(atop(paste('Carbon monoxide'), paste('grams per kilogra', m[fuel]))))
```

```{r figure_2_fuelmass, echo=FALSE, fig.width=12, fig.height=8, dpi=300, dev='png'}

...
```{r, echo=FALSE}
  p1 <-
    ...
    dplyr::mutate(pol_category = "paste(PM[2.5], ' composition')") %>%
      ...
      geom_errorbar(data = std_co, aes(ymax = ymax, ymin = mean), color = "#999999") +
  ...
  p2 <-
    co_p %>%
      ...
      geom_text(data = range_co,
        ...
        annotate("text", x = 7, y = 30, label = "32.5", size = 5) +
        annotate("text", x = 8, y = 30, label = "30.1", size = 5) +
        theme_minimal() +
      geom_linerange(data = std_co, aes(ymax = ymax, ymin = mean), color = "cadetblue") +
...
```{r, echo=FALSE, fig.width=12, fig.height=1.75, dpi=400, dev='pdf'}
  p3 <-
    ...
    dplyr::mutate(pol_category = "paste(PM[2.5], ' composition')") %>%
      ...
      annotate("text", x = 9, y = 15, label = "22.8", size = 5) +
      geom_text(data = range_pm,
                aes(x = stove, y = val, label = sym), size = 10, angle = 90, nudge_x = -0.05) +
      geom_linerange(data = std_pm, aes(ymax = ymax, ymin = ymin)) +
      theme_minimal() +
      theme(legend.text.align = 0,
            text = element_text(size = 18),
            legend.justification = c(1, 1), 
...
```{r, echo=FALSE}
  ...
  ec_oc_ions_p %>%
    dplyr::filter(pol == "organic aerosol") %>%

...
carbonyls_all <-
  ...
  p1 <-
    ...
    ggplot(aes(x = stove, y = val)) +
      ...
      geom_boxplot(data = carbonyls_all, aes(x = stove, y = val), alpha = 0, color = "gray30", width=0.4) +
      geom_point(data = stats_carbs, aes(x = stove, y = min), shape = 24, color = "gray30", fill = "gray30", stroke = 1) +
      geom_point(data = stats_carbs, aes(x = stove, y = max), shape = 25, color = "gray30", fill = "gray30", stroke = 1) +
      theme_minimal() +
...
vocs_all <-
  ...
  p2 <-
    ...
    ggplot(aes(x = stove, y = val)) +
      geom_histogram(aes(fill = pol_subcategory, order = pol_subcategory), stat = "identity", alpha = 0.9) +
      geom_boxplot(data = vocs_all, aes(x = stove, y = val), alpha = 0, color = "gray30", width=0.4) +
      geom_point(data = stat_vocs, aes(x = stove, y = min), shape = 24, color = "gray30", fill = "gray30", stroke = 1) +
      geom_point(data = stat_vocs, aes(x = stove, y = max), shape = 25, color = "gray30", fill = "gray30", stroke = 1) +
...
```{r figure_4, echo=FALSE, fig.width=12, fig.height=8, dpi=400, dev='pdf'}
  p2 + p1 + plot_layout(nrow = 2, heights = c(1, 1))
```

```{r figure_4a, echo=FALSE, fig.width=12, fig.height=2, dpi=400, dev='pdf'}
    vocs_p %>%
    ggplot(aes(x = stove, y = val)) +
      geom_histogram(aes(fill = pol_subcategory, order = pol_subcategory), stat = "identity", alpha = 0.9) +
      geom_boxplot(data = vocs_all, aes(x = stove, y = val), alpha = 0, color = "gray30", width=0.4) +
      ...
      scale_fill_manual(values = c("#fc8d62", "coral3", "slategray2", "#0072B2"),
                        name = "VOCs") +
      #annotate("text", x = 11, y = 300, label = "328", size = 5) +
      scale_y_continuous(limits = c(0, 330), position = "right") +
      theme_minimal() +
      guides(fill = guide_legend(nrow = 2, byrow = FALSE)) +
      theme(text = element_text(size = 18),
            legend.justification = c(1, 1), 
...
```{r figure_4b, echo=FALSE, fig.width=12, fig.height=1.5, dpi=400, dev='pdf'}
  col_vector = c("cornflowerblue",
                 "khaki",
                 "slateblue3",
                 "#9e9ac8")

...
```{r, echo=FALSE}
  p1 <-
    ...
    ylab(expression(atop(paste('Gas-phase Carcinogens'), paste('miligrams per ', MJ[delivered]))))
```
...
```{r figure_5, echo=FALSE, fig.width=12, fig.height=8}
  p2 + p1 + plot_layout(nrow = 2, heights = c(1, 1))
```

```{r figure_5a, echo=FALSE, fig.width=12, fig.height=1.5}
    gases_p %>%
    dplyr::mutate(pol_category = "Gas-Phase Carcinogens") %>%
    ggplot(aes(x = stove, y = val)) +
      geom_histogram(aes(fill = forcats::fct_reorder(pol, mw, .desc = TRUE)), stat = "identity") +
...
```{r figure_5b, echo=FALSE, fig.width=12, fig.height=1.5}
    particles_p %>%
    ggplot(aes(x = stove, y = val)) +
      geom_histogram(aes(fill = pol, order = pol), stat = "identity") +
      geom_text(data = stats_particles  %>%

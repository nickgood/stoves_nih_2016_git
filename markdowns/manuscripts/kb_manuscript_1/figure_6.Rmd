...
```{r}
  ...
  saveRDS(missing_stove, "../../../r_data/model_info.RDS")
        ...
        geom_point(data = pred_metrics2  %>%
                         dplyr::mutate(model = 2,
                                          model =
                                            ...
                                            factor %>% 
                                            forcats::fct_relevel("1",
                                                                 "2")), aes(x = pol, y = rrmse, color = model)) +
        scale_color_manual(values = c("dodgerblue2", "yellowgreen"),
                          labels = c(expression(paste('Model 1: ', PM[2.5], ' and CO')), expression(paste('Model 2: ', PM[2.5], ', CO, and stove type')))) +

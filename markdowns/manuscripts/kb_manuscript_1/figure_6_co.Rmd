...
```{r}
  all <-
    ...
    tidyr::gather("pol", "val", -id, -stove, -co, -fuel) %>%
    dplyr::mutate(co = log(co),
                  val = log(val)) %>%
    na.omit
```
...
```{r}
  ...
  for (n in 1:nrow(pols)) {
    ...
    for(i in 1:nrow(mod_data)) {
      ...
      mdl <- lm(val ~ co, data = training) # model with i'th observation out
...
```{r}
  ...
  for (n in 1:nrow(pols)) {
    ...
    for(i in 1:nrow(mod_data)) {
      ...
      mdl <- lm(val ~ co + stove, data = training) # model with i'th observation out
      y_pred <- predict(mdl, newdata = prediction) # predict i'th observation
      obs_mod_sqdiff[i] <- (prediction$val - y_pred)^2 # obs - model squared
      obs_mean_sqdiff[i] <- (prediction$val - obs_mean)^2 # obs - model squared
...
```{r}
  ...
  for (n in 1:nrow(pols)) {
    ...
    for(i in 1:nrow(mod_data)) {
      ...
      mdl <- lm(val ~ co + fuel, data = training) # model with i'th observation out
      y_pred <- predict(mdl, newdata = prediction) # predict i'th observation
      obs_mod_sqdiff[i] <- (prediction$val - y_pred)^2 # obs - model squared
      obs_mean_sqdiff[i] <- (prediction$val - obs_mean)^2 # obs - model squared
    ...
    dplyr::mutate(model = 1,
      ...
      ggplot(aes(x = reorder(pol, rrmse), y = rrmse, color = model, stroke = 1)) +
        ...
        scale_color_manual(values = c("dodgerblue2", "yellowgreen", "red"),
                          labels = c(expression(paste('Model 1: CO')),
                                     expression(paste('Model 2: CO and stove type')),
                                     expression(paste('Model 3: CO and fuel type')))) +

...
```{r}
  all <-
    ...
    tidyr::gather("pol", "val", -id, -stove, -pm_mass, -fuel) %>%
...
```{r}
  ...
  for (n in 1:nrow(pols)) {
    ...
    for(i in 1:nrow(mod_data)) {
      ...
      mdl <- lm(val ~ pm_mass, data = training) # model with i'th observation out
...
```{r}
  ...
  for (n in 1:nrow(pols)) {
    ...
    for(i in 1:nrow(mod_data)) {
      ...
      mdl <- lm(val ~ pm_mass + stove, data = training) # model with i'th observation out
      y_pred <- predict(mdl, newdata = prediction) # predict i'th observation
      obs_mod_sqdiff[i] <- (prediction$val - y_pred)^2 # obs - model squared
      obs_mean_sqdiff[i] <- (prediction$val - obs_mean)^2 # obs - model squared
...
```{r}
  ...
  for (n in 1:nrow(pols)) {
    ...
    mod_data <-
      all %>%
      dplyr::filter(stove != "forced-draft gasifier") %>%
      dplyr::filter(pol == pols$pol[n])
    ...
    for(i in 1:nrow(mod_data)) {
      ...
      mdl <- lm(val ~ pm_mass + fuel, data = training) # model with i'th observation out
      y_pred <- predict(mdl, newdata = prediction) # predict i'th observation
      obs_mod_sqdiff[i] <- (prediction$val - y_pred)^2 # obs - model squared
      obs_mean_sqdiff[i] <- (prediction$val - obs_mean)^2 # obs - model squared
...
```{r figure_6, fig.height=5, fig.width=12, dpi=400, dev='pdf'}
  #p1 <-
    pred_metrics %>%
    dplyr::mutate(model = 1,
      ...
      ggplot(aes(x = reorder(pol, rrmse), y = rrmse, color = model, stroke = 1)) +
        ...
        scale_color_manual(values = c("dodgerblue2", "yellowgreen", "red"),
                          labels = c(expression(paste('Model 1: ', PM[2.5])),
                                     expression(paste('Model 2: ', PM[2.5], ' and stove type')),
                                     expression(paste('Model 3: ', PM[2.5], ' and fuel type')))) +

...
```{r}
  tibble::data_frame("Smoke constituent" = c("Fine particulate matter",
                     ...
                     "Analysis instrumentation" = c("MX5 Microbalance; Mettler Toledo; Columbus, OH",
                                                    ...
                                                    "ULTRAMAT/OXYMAT 6 and ULTRAMAT 23; Siemens; Munich, Germany"),
...
```{r}
  tibble::data_frame("Instrumentation" = c("Hot-wire anemometer",
                     ...
                     "Details" = c("Velometer AVM430-A; TSI Alnor; Huntington Beach, CA",
                                   ...
                                   "K-type, OMEGA Engineering Inc., Norwalk, CT",
                                   "",
                                   "",
                                   ""),
...
# SI 10: Models used in selection

The following are equations of the models used in the small-sample-size-corrected Akaike information criterion (AICc) selection analysis, where $\alpha$, $\beta$, and $\gamma$ are constant coefficients; $\alpha_i$, $\beta_i$, and $\gamma_i$ are stove-type or fuel-type specific coefficients; and $CO$ is carbon monoxide, $PM_{2.5}$ is fine particulate matter, and $EF$ is the smoke constituent that is being predicted on a per-energy-delivered basis.

```{r}
  tibble::data_frame("Model equations" = c("$ln(EF) = \\alpha_i$",
                                           "$ln(EF) = \\alpha + \\beta \\cdot PM_{2.5}$",
                                           "$ln(EF) = \\alpha + \\gamma \\cdot CO$",
                                           "$ln(EF) = \\alpha + \\beta \\cdot PM_{2.5} + \\gamma \\cdot CO$",
                                           "$ln(EF) = \\alpha_i + \\beta \\cdot PM_{2.5}$",
                                           "$ln(EF) = \\alpha_i + \\gamma \\cdot CO$",
                                           "$ln(EF) = \\alpha_i + \\beta \\cdot PM_{2.5} + \\gamma \\cdot CO$",
                                           "$ln(EF) = \\alpha_i + \\beta_i \\cdot PM_{2.5}$",
                                           "$ln(EF) = \\alpha_i + \\gamma_i \\cdot CO$",
                                           "$ln(EF) = \\alpha_i + \\beta_i \\cdot PM_{2.5} + \\gamma \\cdot CO$",
                                           "$ln(EF) = \\alpha_i + \\beta \\cdot PM_{2.5} + \\gamma_i \\cdot CO$",
                                           "$ln(EF) = \\alpha_i + \\beta_i \\cdot PM_{2.5} + \\gamma_i \\cdot CO$")) %>%
    kable("latex", escape = FALSE,  booktabs = TRUE, align = 'c',
          linesep = "") %>%
...
# SI 11: Number of measurements by stove type
...
# SI 12: Fraction of measurements below limit of detection
...
# SI 13: Fraction of measurements below background
...
## SI 13.2: Pellet, charcoal, and fossil-fuel stoves
...
# SI 15: Top models by pollutant (with stove type as categorical variable)

*** Will add if JV & JT like new regression analysis

# SI 16: Top models by pollutant (with fuel type as categorical variable)

...
```{r}
  three_stone <-
    ...
    dplyr::summarise(val = 0.1)  %>%
    dplyr::filter(!grepl("organic carbon", pol_subcategory)) %>%
    dplyr::mutate(pol_subcategory = as.character(pol_subcategory),
                  ...
                  pol_subcategory = ifelse(grepl("^co$", pol_subcategory), "carbon monoxide", pol_subcategory),
                  pol_subcategory = ifelse(pol_subcategory == "pm_mass", "fine particulate matter", pol_subcategory),
...
```{r, fig.width=10, fig.height=10}
  wordcloud2(three_stone, figPath = "images/cloud.jpg", size = 0.3)
```

...
```{r}
  batch_mass <- dplyr::filter(batch, var == "fuel" |
                ...
                dplyr::rename(wgt_fuel = fuel,
                              wgt_refuel = refueled,
                              wgt_pot = ashpot,
                              wgt_pot_ash = ashandpot) %>%
                dplyr::mutate(mass_fuel = wgt_fuel + wgt_refuel - (wgt_pot_ash - wgt_pot))
```
...
```{r}
  wood_mass <- dplyr::filter(wood, var == "fuel" |
                ...
                dplyr::mutate(mass_fuel = wgt_fuel -
                                          (wgt_pot_unusedfuel - wgt_pot) - 
                                          (wgt_pot_char_ash - wgt_pot))
```
...
```{r}
  fuel_burnt <- dplyr::bind_rows(dplyr::select(batch_mass, id, mass_fuel),
                ...
                dplyr::filter(!grepl("^27|^28|^29|^30", id), !is.na(mass_fuel))
```

...
* G2: filter ruptured, not sure which one - check if outlier
...
```{r}
  ...
  saveRDS(carbs_lod, file = "../../r_data/carbohydrates_lod.RDS")
```
...
notes: backgrounds look good - no backgrounds were outliers across multiple pollutants - proceed
...
notes: a few outliers - see how they impact background average
...
notes: many backgrounds appear to be at the limit of detection
...
```{r, echo=FALSE}
  carbohyrates_merged_p %>%
    ...
    dplyr::mutate(z_score = (val - mean(val, na.rm = TRUE))/ sd(val, na.rm = TRUE)) %>%
    dplyr::ungroup() %>%
    dplyr::select(id, pol, stove, z_score) %>%
    dplyr::filter(z_score > 3) %>%
    knitr::kable("markdown", align = 'c', digits = 2)
```

notes: 2A was one of the outliers to look for, but since only one pollutant is an outlier, leave for now

## table: outliers by stove type (summed)
...
notes: 2A is an outlier again - fire went out for this test - leave for now
...
```{r, echo = FALSE}
  dplyr::select(samples, id, stove, fuel) %>%
  dplyr::anti_join(carbohyrates_merged %>%
                     dplyr::distinct(),
                   by = "id") %>%

  ...
  ecoc <- 
    ecoc %>%
      dplyr::select(id, oc_ug_sq_cm, ec_ug_sq_cm,
                    cassette, test_type, oc_unc, ec_unc, fid2, punch_area, qc) %>%
...
* G5: ignore
* 6A: removed originally for too high of loading - going to analyze filter again (couldn't get to work with half punch)
...
```{r}
  ecoc <- 
    ...
    dplyr::mutate(qc = as.character(qc),
                  qc = ifelse(grepl("24A|^3A|G5", id), "bad", qc),
...
## take only last value for each id chronologically

```{r}
  ecoc_merged <-
    ecoc %>%
    dplyr::group_by(id, cassette, pol, test_type) %>%
...
```{r}
  readr::write_csv(ecoc_merged %>%
                   dplyr::ungroup() %>%
                   tidyr::spread(pol, val) %>%
                   dplyr::left_join(flows, by = c("id", "cassette")) %>%
                   dplyr::left_join(times, by = c("id", "cassette")) %>%
                   dplyr::mutate(cassette = ifelse(cassette == "e", "bq", "qbt")),
                   "../../exported_data/oc_raw.csv")
```

# uncertainty

## plot: ratio of uncertainty to value
...
```{r, echo=FALSE, fig.width=12, fig.height=5}
  ecoc_merged %>%
    dplyr::ungroup() %>%
    dplyr::filter(test_type == "test", grepl("ec", pol), cassette == "e") %>%
    dplyr::select(id, cassette, qc, pol, val) %>%
    tidyr::spread(pol, val) %>%
    dplyr::mutate(ratio = ec_unc / ec) %>%
    dplyr::left_join(samples %>%
                     dplyr::select(id, stove, fuel, fuelcat, date), by = "id") %>%
      ggplot(aes_string(x = "id", y = "ratio")) +
      geom_point(size = 2) +
      theme_bw() +
...
```{r, echo=FALSE, fig.width=12, fig.height=5}
  ecoc_merged %>%
    dplyr::filter(test_type == "test", grepl("oc", pol), cassette == "e") %>%
    dplyr::select(id, cassette, qc, pol, val) %>%
    tidyr::spread(pol, val) %>%
    dplyr::mutate(ratio = oc_unc / oc) %>%
    dplyr::left_join(samples %>%
                     dplyr::select(id, stove, fuel, fuelcat, date), by = "id") %>%
      ggplot(aes_string(x = "id", y = "ratio")) +
      geom_point(size = 2) +
...
notes: all less than 1% uncertainty

filter out uncertainty 
...
```{r}
  ecoc_merged <-
    ecoc_merged %>%
    dplyr::mutate(val = val * filter_area)
```
...
```{r}
  blanks <-
    ecoc_merged %>%
    dplyr::filter(test_type == "blank", !grepl("unc", pol))
```
...
## save data for volatility (Shantanu)

```{r}
  readr::write_csv(blanks %>%
                     dplyr::ungroup() %>%
                     dplyr::select(-cassette) %>%
                     dplyr::filter(pol == "oc"),
                   "../../exported_data/oc_blanks_disaggregated.csv")
```
...
```{r}
  readr::write_csv(blanks %>%
                     dplyr::filter(pol == "oc"),
                   "../../exported_data/oc_blank_summary.csv")
```
...
```{r}
  ecoc_merged <-
    ecoc_merged %>%
    dplyr::left_join(dplyr::select(blanks, pol, mean), by = "pol") %>%
    dplyr::mutate(val = ifelse(!grepl("unc", pol), val - mean, val)) %>%
    dplyr::select(-mean)
```
...
```{r}
  ecoc_merged <-
    ...
    dplyr::mutate(lod = ifelse(val <= lod_val & !grepl("unc", pol), "below", "above"),
                  val = ifelse(lod == "below", lod_val / sqrt(2), val),
                  lod = ifelse(grepl("unc", pol), NA, lod)) %>%
...
## plot: ratio of ec on artifact filters to front filters

```{r, echo = FALSE}
  ecoc_merged %>%
    dplyr::filter(cassette == "e", pol == "ec", test_type == "test") %>%
    dplyr::left_join(ecoc_artifact, by = c("id", "pol")) %>%
    dplyr::mutate(ratio = artifact / val) %>%
    dplyr::filter(ratio > 0.03) %>%
    ggplot(aes_string(x = "id", y = "ratio")) +
      geom_point(aes(color = qc), size = 2) +
      scale_color_manual(values = qc_colors) +
      theme_bw() +
      theme(text = element_text(size = 10),
            ...
            legend.position = "top") +
      facet_wrap(~test_type, scales = "free", ncol = 1) +
      ylab("ratio (artifact to loaded filter)")
```

notes: high ratios imply break through
...
```{r}
  ecoc_merged <-
    ecoc_merged %>%
    dplyr::mutate(val = ifelse(cassette == "a" & grepl("ec", pol), 0, val))
```
...
## plot: time series of lab backgrounds (by filter type)

```{r ecoc_bg_timeseries_byfilter, echo=FALSE}
  bg %>%
    dplyr::filter(!grepl("unc", pol)) %>%
...
```{r}
  bg <-
    ...
    dplyr::ungroup() %>%
    dplyr::select(-cassette) %>%
    dplyr::left_join(bg_artifact, by = c("id", "pol")) %>%
    dplyr::mutate(val = ifelse(!grepl("unc", pol), val - artifact, val),
                  val = ifelse(grepl("unc", pol), sqrt(val^2 + artifact^2), val)) %>%
    dplyr::filter(val > 0)
```
...
```{r}
  readr::write_csv(ecoc_merged %>%
                     dplyr::ungroup() %>%
                     dplyr::filter(pol == "oc") %>%
                     dplyr::select(id, test_type, cassette, val, qc, lod) %>%
                     tidyr::spread(cassette, val) %>%
                     dplyr::rename("qbt" = "a",
                                   "bq" = "e") %>%
                     dplyr::left_join(dplyr::select(samples, id, stove, fuel), by = "id"),
                   "../../exported_data/oc.csv")
```
...
```{r}
  ecoc_merged %>%
    dplyr::filter(cassette == "e" & pol == "oc") %>%
    dplyr::left_join(ecoc_artifact, by = c("id", "pol")) %>%
    dplyr::mutate(ratio = artifact / val) %>%
      ggplot(aes(id, ratio)) + 
        geom_point() +
...
```{r}
  ecoc_merged <-
    ...
    dplyr::select(-cassette) %>%
    dplyr::left_join(ecoc_artifact, by = c("id", "pol")) %>%
    dplyr::mutate(artifact = ifelse(is.na(artifact) & pol == "ec", 0, artifact),
                  artifact = ifelse(is.na(artifact) & pol == "ec_unc", 0, artifact),
                  artifact = ifelse(is.na(artifact) & pol == "oc", val * oc_ratio$ratio[3], artifact),
                  artifact = ifelse(is.na(artifact) & pol == "oc_unc", val * oc_ratio$ratio[4], artifact),
                  val = ifelse(!grepl("unc", pol), val - artifact, val),
                  val = ifelse(grepl("unc", pol), sqrt(val^2 + artifact^2), val)) %>%
...
```{r}
  ecoc_merged <-
   ecoc_merged %>%
    dplyr::mutate(val = ifelse(!grepl("unc", pol), val - median, val)) %>%
...
notes: two tests 26A and 25B were below background for oc
...
```{r, echo=FALSE}
  ecoc_merged_p %>%
    ...
    dplyr::mutate(z_score = (val - mean(val, na.rm = TRUE)) / sd(val, na.rm = TRUE)) %>%
    dplyr::ungroup() %>%
    dplyr::select(id, stove, fuel, pol, z_score) %>%
    dplyr::filter(z_score > 3) %>%
    knitr::kable("markdown", align = 'c')
```
...
* 1A: removed for too high of loading
* 24A: possibly ruptured - check this
* 3A: flow measurements unknown

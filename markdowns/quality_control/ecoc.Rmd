...
* 4C: possible issues with flow rate, artifact filter blew out, possibly after test was over - check if outlier
* 8A: filter ruptured, not sure which one - check if outlier
* 8C: potential problems with whole test - check if outlier
* 17A: quartz filter ruptured, not sure which one - check if outlier
...
## filter ec artifact filters

```{r}
  ecoc_merged <-
    ecoc_merged %>%
    dplyr::filter(!(cassette == "a" & pol == "ec"))
```
...
```{r}
  bg <-
    ...
    dplyr::mutate(artifact = ifelse(pol == "ec", 0, artifact)) %>%
    dplyr::mutate(val = val - artifact) %>%
    dplyr::select(-artifact, -cassette)
```

note: several oc values are negative once corrected for artifact - leave for now

* when backgrounds are below lod and negative - could be negative due to measurement uncertainty 
* when backgrounds are above lod and negative - could be negative because quartz behind teflon filter method tends to over estimate positive absorption artifact due to volatilization of organic gas off of teflon filter
...
```{r}
  ecoc_merged <-
    ...
    dplyr::left_join(ecoc_artifact, by = c("id", "pol")) %>%
    dplyr::mutate(artifact = ifelse(pol == "ec", 0, artifact),
                  artifact = ifelse(is.na(artifact), val * 0.26, artifact)) %>%
    dplyr::mutate(val = val - artifact) %>%
    dplyr::select(-artifact, -cassette) %>%
    dplyr::select(-lod_a)
```
...
notes: one test 26A is below background, but above lod - need to decide what to do about this measurement

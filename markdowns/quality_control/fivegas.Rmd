...
notes: data is expected to be missing for: 2A, 4A, and 1B - look into 17C missing
...
## remove bad tests and NAs

```{r}
  fivegas_merged <-
    fivegas_merged %>% 
    dplyr::filter(qc != "bad", !is.na(val))
```

## remove or replace negatives

* several large negative values (approx. -150 ppm for ch4 in test 13A) - remove
* small negative values (approx. 0 in test 13A and 23A) - set to zero

```{r}
  fivegas_merged <-
    fivegas_merged %>%
    dplyr::mutate(val = ifelse(val < -100, NA, val),
...
```{r, echo=FALSE}
  fivegas_merged %>%
    ...
    dplyr::filter(prct_missing >= 25) %>%
    dplyr::select(id, prct_missing) %>%
    knitr::kable("markdown", align = 'c', digits = 2)
```

notes: 11A is missing approx. 48% of test missing - remove

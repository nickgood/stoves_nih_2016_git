...
```{r fivegas_volts_trace_co2, echo=FALSE, fig.width=12}
 fivegas_volts %>%
...
```{r fivegas_volts_trace_co, echo=FALSE, fig.width=12}
 fivegas_volts %>%
...
```{r fivegas_volts_trace_ch4, echo=FALSE, fig.width=12}
 fivegas_volts %>%
...
* 23A: signal is bad for all pollutants - remove
...
remove NAs are replace negatives with zero

* notes: several small negative values in data set - set to zero
...
## replace negatives with "BB" for below background

* 23B and 4B have large negative values after subtracting background (> -100 ppm) - talk to John about what to do here
* small negative values (approx. -10-0 ppm, mostly methane data) - set to zero

```{r}
  fivegas_merged <-
    fivegas_merged %>%
    dplyr::mutate(val = ifelse(val < 0, "BB", val))
```
...
```{r}
  fivegas_merged <-
    ...
    dplyr::mutate(val = convert_ppmv_ugpmc(val, mw, 25, 85),
...
```{r}
  fivegas_merged <-
    fivegas_merged %>%
    dplyr::select(id, datetime, pol, val, qc, units) %>%
...
```{r}
  fivegas_merged <-
    ...
    dplyr::left_join(sample_times %>%
                     dplyr::mutate(dur = end - start) %>%
                     dplyr::select(id, dur), by = "id") %>%
    dplyr::select(id, pol, val, dur, qc) %>%
    dplyr::mutate(lod = NA,
                  pol_category = pol)

...
```{r}
  pah <-
    ...
    dplyr::mutate(id = as.character(id),
                  id = ifelse(id == "10D", "1D", id),
                  id = ifelse(id == "8E", "6E", id))
```

notes: email from Pierre saying 10D-B = 1D-B and 8E-B = 6E-B
...
* G6-B: lost somewhere along the line
* one other, not sure which one?

## remove tests with measurement missing
...
notes: asking John which (if any) missing filter type would make a measurement bad
...
notes: knowing this is not the best way to deal with them - remove for now and assume they are equal to zero - in contact with Pierre regarding the best approach to deal with these data
...
## plot: time series of burns (back puf)

```{r pah_burn_timeseries_b, echo=FALSE, fig.width=12}
  pah_merged %>%
    dplyr::filter(filter_type == "back puf") %>%
    dplyr::filter(!is.na(val)) %>%
    dplyr::left_join(dplyr::select(samples, id, date), by = 'id') %>%
...
## plot: time series of burns (front puf)

```{r pah_burn_timeseries_f, echo=FALSE, fig.width=12}
  pah_merged %>%
    dplyr::filter(filter_type == "front puf") %>%
    dplyr::filter(!is.na(val)) %>%
    dplyr::left_join(dplyr::select(samples, id, date), by = 'id') %>%
...
## plot: time series of burns (filter)

```{r pah_burn_timeseries_p, echo=FALSE, fig.width=12}
  pah_merged %>%
    dplyr::filter(filter_type == "front puf") %>%
    dplyr::filter(!is.na(val)) %>%
    dplyr::left_join(dplyr::select(samples, id, date), by = 'id') %>%
...
```{r}
  pah_merged <-
    ...
    dplyr::select(id, pol, val, dur, lod, qc, pol_category)

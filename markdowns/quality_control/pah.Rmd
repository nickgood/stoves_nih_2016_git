...
```{r}
  ...
  pah_duplicates <- readRDS("../../r_data/pah_duplicates.RDS")    # pah dataset
...
```{r}
  lod <-
    ...
    dplyr::filter(id != "10D", id != "8E",
                  ...
                  !(filter_type == "f" & grepl("24A", id) & pol == "fluorene"),
                  !(filter_type == "f" & grepl("16C|23B", id) & pol == "phenanthene")) %>%
    dplyr::filter(val == "BDL", !grepl("^27|^28|^29|^30", id)) %>%
    dplyr::mutate(lod = "below") %>%
    dplyr::select(id, pol, filter_type, lod)
```
...
```{r}
  zero_vals <-
    pah %>%
      dplyr::filter(val == 0) %>%
      dplyr::arrange(id, pol)

  readr::write_csv(zero_vals, "../../qaqc_files/zero_vals.csv")
```
...
```{r}
  pah <-
    pah %>%
    dplyr::anti_join(duplicates,
                     by = c("id", "pol", "filter_type")) %>%
    dplyr::bind_rows(pah_duplicates %>%
                       dplyr::mutate(id = ifelse(id == "10D", "1D", id),
                                     id = ifelse(id == "8E", "6E", id)))
```
...
## rename two mislabeled pollutants

```{r}
  pah <-
    pah %>%
    dplyr::mutate(pol = ifelse(pol == "napthalene", "naphthalene", pol),
                  pol = ifelse(pol == "phenanthene", "phenanthrene", pol))
```
...
```{r}
  pah_merged <-
    pah %>% 
    dplyr::filter(qc != "bad") %>%
    dplyr::filter(id != "14C", id != "24A") 
```

notes: 
- remove 14C - quartz filter missing (experimental contamination)
- remove 24A - quartz filter had bad run (Pierre plans to rerun)

# manually fix NAs based on Pierre's email

```{r}
  manual_vals <-
    tibble::data_frame(pol = c("fluorene", "fluorene", "fluorene"),
                       id = c("18B", "19A", "9A"),
                       filter_type = c("filter", "filter", "filter"),
                       val = c(0.07 / sqrt(2), 0.145 / sqrt(2), 0.215 / sqrt(2)),
                       lod = c("below", "below", "below"),
                       qc = c("ok", "ok", "ok"),
                       test_type = c("test", "test", "test"))


  pah_merged <-
    pah_merged %>%
    dplyr::bind_rows(manual_vals)
```
...
```{r}
  all_vals <-
    pah_merged %>%
    dplyr::select(id, pol) %>%
    dplyr::full_join(dplyr::select(samples, id), by = "id") %>%
    dplyr::filter(!grepl("^G", id), !grepl("^B", id), !grepl("^27|^28|^29|^30", id)) %>%
    dplyr::distinct() %>%
    tidyr::expand(id, pol)
```
...
```{r}
  missing_filters <-
    ...
    dplyr::anti_join(pah_merged %>%
                       ...
                       dplyr::filter(filter_type == "filter"), by = c("id", "pol")) %>%
  dplyr::filter(!grepl("14C|21A", id)) %>%
...
```{r}
  missing_front_pufs <-
    ...
    dplyr::anti_join(pah_merged %>%
    dplyr::filter(!grepl("^G", id), !grepl("^B", id), !grepl("^27|^28|^29|^30", id)) %>%
    dplyr::filter(filter_type == "front puf"), by = c("id", "pol")) %>%
  ...
  missing_back_pufs <-
    ...
    dplyr::anti_join(pah_merged %>%
    dplyr::filter(!grepl("^G", id), !grepl("^B", id), !grepl("^27|^28|^29|^30", id)) %>%
    dplyr::filter(filter_type == "back puf"), by = c("id", "pol")) %>%
    dplyr::mutate(id = gsub("$", "-B", id))
  
  readr::write_csv(missing_filters %>%
                     dplyr::bind_rows(missing_front_pufs) %>%
                     dplyr::bind_rows(missing_back_pufs) %>%
                     dplyr::arrange(id, pol),
                   "../../qaqc_files/missing_test_filters.csv")
```
...
```{r}
  pah_lod <-
    blanks %>%
    dplyr::select()
    tidyr::spread(filter_type, pol) %>%
    dplyr::mutate(lod = paste0(filter, "|", puf))

  saveRDS(pah_lod, "../../r_data/pah_lod.RDS")
```
...
```{r}
  pah_merged <-
    ...
    dplyr::left_join(dplyr::select(blanks, pol, mean, lod, filter_type) %>%
                     dplyr::rename("lod_val" = "lod",
                                   "filter_class" = "filter_type"), by = c("pol", "filter_class")) %>%
    dplyr::mutate(val = val - mean,
                  lod = ifelse(val <= lod_val, "below", "above"),
                  val = ifelse(val <= lod_val, lod_val / sqrt(2), val)) %>%
    dplyr::select(-lod_val, -filter_class, -mean)
```

```{r, eval=FALSE}
  # total number 
  pah_merged %>%
    dplyr::filter(lod == "above", filter_type == "filter") %>%
    nrow(.)

  # lod 
  pah_merged %>%
    dplyr::filter(lod == "below", filter_type == "filter") %>%
    nrow(.)
```
...
```{r, eval=FALSE}
  pah_merged %>%
    dplyr::filter(test_type == "test", grepl("front puf|filter", filter_type)) %>%
    tidyr::spread(filter_type, val) %>%
    dplyr::rename("front_puf" = "front puf") %>%
    dplyr::mutate(ratio = front_puf / (filter + front_puf)) %>%
      ggplot(aes_string(y = "ratio", x = "pol")) +
      geom_boxplot(outlier.colour = NA) +
      ...
      ylab("fraction of gas phase PAHs") +
      xlab("")
```

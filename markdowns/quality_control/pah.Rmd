...
average in case there are duplicate measurements
...
```{r, echo=FALSE}
 blanks %>%
    ...
    ggplot(aes_string(x = "date", y = "val")) +
      ...
      facet_wrap(pol~filter_type, ncol = 4, scales = "free") +
...
```{r pah_burn_timeseries, echo=FALSE}
  pah_merged %>%
    ...
    ggplot(aes_string(x = "date", y = "val")) +
      ...
      theme(text = element_text(size = 16),
            legend.position = "top") +
      facet_wrap(pol~filter_type, scales = "free", ncol = 3) +
      ylab("concentration (microgram per meter cubed)") +
      xlab("")
```
...
```{r pah_burn_bg_conc, echo=FALSE}
  pah_merged %>%
    ...
    ggplot(aes_string(y = "val", x = "test_type")) +
      ...
      theme(text = element_text(size = 16),
            legend.position = "top") +
      facet_wrap(pol~filter_type, scales = "free", ncol = 2) +
      ylab("concentration (microgram per meter cubed)") +
      xlab("")
```
...
## replace low values with zero and sum all filters
...
```{r, echo=FALSE}
  pah_merged <-
    ...
    dplyr::select(id, pol, dur, filter_type, val) %>%
    tidyr::spread("filter_type", "val") %>%
    dplyr::filter(!is.na("front puf"), !is.na("filter")) %>%
    tidyr::gather("filter_type", "val", 4:6) %>%
...
```{r}
  pah_merged <-
    pah_merged %>%
    dplyr::mutate(pol_category = "pahs", lod = NA) %>%

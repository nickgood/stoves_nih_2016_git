...
```{r}
  duplicate_vals <-
    pah %>%
      dplyr::inner_join(duplicates, by = c("id", "pol", "filter_type")) %>%
      dplyr::group_by(id, pol, filter_type) %>%
      dplyr::summarise(prct_change = (max(val) - min(val) / max(val)) * 100) %>%
      dplyr::filter(prct_change > 5 | prct_change < -5) %>%
      dplyr::select(-prct_change) %>%
      dplyr::distinct() %>%
      dplyr::inner_join(pah, by = c("id", "pol", "filter_type"))

  readr::write_csv(duplicate_vals, "../../qaqc_files/duplicate_vals.csv")
```

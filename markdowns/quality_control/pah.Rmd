...
```{r}
  saveRDS(blanks, "../../r_data/pah_lod.RDS")
```
...
```{r}
  filters <-
    pah_merged %>%
    dplyr::filter(test_type == "test", filter_type =="filter") %>%
    dplyr::group_by(id, pol)  %>%
    dplyr::summarise(filter = sum(val, na.rm = TRUE))
```
...
```{r PAH_partion, fig.width=12, fig.height=4}
  #test <-
  pah_merged %>%
    dplyr::left_join(filters, by = c("pol", "id")) %>%
    na.omit() %>%
    dplyr::filter(lod !="below") %>%
    dplyr::mutate(ratio = filter / (val)) %>%
      ...
      theme(text = element_text(size = 12),
            legend.position = "top",
            axis.text.x = element_text(angle = 45, hjust = 1, vjust = 0.75)) +
      ylab("Fraction of particle-phase PAHs") +
      xlab("")
```

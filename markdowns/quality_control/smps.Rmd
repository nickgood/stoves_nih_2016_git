...
notes: scans with high number concentration - investigate
...
```{r}
  smps_merged <-
    ...
    dplyr::select(id, scan, pol, size, val, qc) %>%
...
```{r}
  ultrafines <-
    ...
    dplyr::group_by(id) %>%
    dplyr::summarise(val = sum(val) * smps_constant,
                     qc = first(qc)) 
```

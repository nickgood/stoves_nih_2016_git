...
all errors only happened for one or two scans - remove those scans
...
```{r smps_burn_bg_conc, echo=FALSE}
  smps_merged %>%
    dplyr::filter(grepl("24|25|26" ,id)) %>%
    dplyr::filter(!is.na(val)) %>%
    dplyr::left_join(dplyr::select(samples, id, date), by = 'id') %>%
    dplyr::group_by(id, scan, test_type) %>%
    dplyr::summarise(val = sum(val, na.rm = TRUE),
...
```{r}
  smps_merged <-
    smps_merged %>%
    dplyr::group_by(id, pol, pol_category, size) %>%
    dplyr::summarise(val = mean(val, na.rm = TRUE),
                     qc = last(qc),
                     bg = last(bg))

  saveRDS(smps, file = "../../r_data/smps_merged_avg.RDS")
```
...
```{r}
  ultrafines <-
    ...
    dplyr::left_join(smps_times, by = "id") %>%

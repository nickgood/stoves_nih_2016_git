...
## assume any blank cells are below lod

```{r}
  vocs <-
    vocs %>%
    dplyr::mutate(val = ifelse(is.na(val), -8888, val))
```

notes: assume any blank values are below lod - email Arsineh to confirm
...
* 2A: fire went out during test, flow may have been low on canister - check if outlier
* 2C: start up period twice - check if outlier
* 8C: many problems - check if outlier
* 23A: fire relit with kerosene during reful - check if outlier
* G8: possible leak problem - check if outlier
...
```{r}
  test <-
    ...
    dplyr::mutate(val = convert_ppbv_ugpmc(val, mw),
...
notes: none are outliers across pollutants - proceed

## plot: time series of lab backgrounds


```{r vocs_bg_timeseries, echo=FALSE, fig.width=12, fig.height=20}
 bg %>%
...
```{r ions_test_bg_conc, echo=FALSE, fig.width=12, fig.height=20}
  vocs_merged %>%
...
notes: need to investigate ways to account for background measurements (for now, subtract the median)
...
```{r vocs_samples_timeseries, echo=FALSE, fig.width=12, fig.height=20}
 vocs_merged %>%
    dplyr::filter(test_type == "test") %>%
    dplyr::filter(!is.na(val)) %>%
    dplyr::left_join(dplyr::select(samples, id, date), by = 'id') %>%
...
```{r vocs_stovetype, echo=FALSE, fig.height=50}
  vocs_merged_p <-
    vocs_merged %>%
...
notes: not an outlier of interest, proceed for now

## plot: concentrations by fuel type

```{r vocs_fueltype, echo=FALSE, fig.height=50}
  vocs_merged_p %>%
...
notes: 2E seems to be appearing the most - proceed for now

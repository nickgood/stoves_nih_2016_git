...
```{r}
  corr_map <-
    ...
    dplyr::select(id, acetaldehyde, benzene, co, ec, formaldehyde, ions, oc, pm2.5)
```
...
```{r}
 corr_map <- corr_map %>%
             dplyr::select(id, pm2.5, acetaldehyde, benzene, formaldehyde, ec) %>%
...
```{r}
  ...
  ggplot(corr_map, aes(x = pm2.5, y = value, color = stovecat)) +
    ...
    xlab("Particulate Matter 2.5 (mg emitted per MJ fuel)")
```

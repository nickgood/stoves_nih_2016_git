...
Merge with stove/fuel data to allow better plotting.

```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  samples_sf <- dplyr::select(samples, id, stove, fuel)%>%
            dplyr::mutate(id= as.factor(id))

  background_means_p <- dplyr::left_join(background_means, samples_sf, by = "id") %>%
            dplyr::mutate(id= as.factor(id))
...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE, fig.width = 12, fig.height=8}
  ggplot(background_means_p, aes(id, mean, colour = type)) +
  geom_point() +
  ggtitle("CH4") +
  theme_minimal()

  ggplot(background_means_p, aes(x= interaction(stove, fuel), mean, colour = type)) +
  geom_point() +
  ggtitle("CH4") +
  theme_minimal() +

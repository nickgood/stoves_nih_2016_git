...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  mean_bg <- dplyr::filter(background_means, qc == "ok" & type != "sample") %>%
	           dplyr::group_by(id) %>%
	           dplyr::summarise(mean = mean(mean, na.rm = TRUE)) %>%
	           dplyr::ungroup() %>%
             dplyr::summarise(mean = mean(mean, na.rm = TRUE)) %>%
	           dplyr::mutate(id = NA, type = "avg", qc = NA)

  background_means <- dplyr::bind_rows(background_means, mean_bg)
```

## Save to file

```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  save(background_means, file = "../r_files/background_co2.Rda")
```

...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  benzene <- dplyr::mutate(benzene, pol_fuel_mass_ratio = mass_emitted * 1e-6 / mass_fuel)  # ug * 1e-6 / g
```
...
ggplot(benzene, aes(x= id, pol_fuel_mass_ratio, colour = fuelcat)) +
  ...
  xlab("Test ID") +
  ylab("ug emitted / kg fuel burned") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45))
```

```{r, echo=FALSE, warning=FALSE, message=FALSE, results='markup', cache=FALSE, fig.width = 10}
ggplot(benzene, aes(x= stove, pol_fuel_mass_ratio, colour = fuelcat)) +
  geom_point() +

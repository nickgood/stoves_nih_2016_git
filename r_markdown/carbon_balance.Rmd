...
```{r}
  ...
  df_bg        <- readRDS("../r_files/fivegas_background.RDS")
```
...
```{r}
  ...
  data_fuel      <- readRDS("../r_files/data_fuel_prep_clean.RDS")
...
Get the background concentration for each pollutant for each test.

```{r}
  df_fivegas <- dplyr::left_join(df_fivegas,
                                 dplyr::select(df_bg, id, pol, ppm_bg, conc_bg),
                                 by=c("pol","id"))
```
...
```{r, warning=FALSE}
  ...
  df_fuel <- dplyr::group_by(data_fuel, id)              %>%
             dplyr::summarise(m_fuel_dry = sum(mass_dry),
...
```{r}
  df_fuel <- df_fuel %>%
             dplyr::mutate(m_shims_dry  = m_shims_wet/(1+mc_low_dry),
                           ...
                           y_c_char     = 0.95) # assumed mass fraction of carbon in char
```
...
```{r, fig.width=3.15, fig.height=3.15, fig.align='center'}
  ...
  ggplot(df_c_bal, aes(ratio)) +
    ...
    geom_histogram(na.rm=TRUE, breaks=seq(0.825,1.175,0.05),
                   color="black") +
    coord_cartesian(xlim=c(0.8,1.2), ylim=c(0,12)) + 

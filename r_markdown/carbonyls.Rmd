...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  load("../r_files/ions.Rda")  # loads carbonyls

  carbonlys <- dplyr::select(ions, 24:39, type, id, id_ions)  # select carbonlys only
```
...
Carbonyl data was collected during `r length(unique(carbonyls_merged$id))` experiments. There is no $ion$ data for tests: `r setdiff(as.character(samples$id), as.character(carbonyls_merged$id))`.

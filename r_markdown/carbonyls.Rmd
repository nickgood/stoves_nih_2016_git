...
Replace `NA` values and values below the LOD with LOD/$\sqrt(2)$.

```{r}
  df_carb <- dplyr::mutate(df_carb,
                           mass_ug = ifelse(mass_ug < lod,  lod/sqrt(2), mass_ug),
                           mass_ug = ifelse(is.na(mass_ug), lod/sqrt(2), mass_ug))
```
...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  df_bg <- dplyr::filter(df_carb, substr(id,1,2) == "BG", qc != "bad") %>%
           dplyr::group_by(pol) %>%

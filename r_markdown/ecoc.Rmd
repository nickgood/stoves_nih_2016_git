...
```{r, echo=FALSE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  ...
  kable(loq_ec, align = 'c', caption = "Filters below LOD for EC")
```
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  ecoc_calcs <- dplyr::mutate(ecoc_merged, ec_ug = ec_ug_sq_cm * area) %>%
                dplyr::mutate(oc_ug = oc_ug_sq_cm * area) %>%
                dplyr::mutate(time = (end - start) / 60) %>%
                dplyr::mutate(flow = ((post_flow + pre_flow) / 2) / 1000) %>%

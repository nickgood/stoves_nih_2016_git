...
There were `r ec_lod_loq$n` blank ecoc filter analyzed. The average blank oc was `r round(oc_lod_loq$mean, 3)` [ug/sq.cm] the average blank ec was `r round(ec_lod_loq$mean), 3)` [ug/sq.cm]. For OC the limit of detection was `r round(oc_lod_loq$lod, 3)` [ug/sq.cm] and the limit of quantification was `r round(oc_lod_loq$loq, 3)` [ug/sq.cm]. For EC the limit of detection was `r round(ec_lod_loq$lod, 3)` [ug/sq.cm] and the limit of quantification was `r round(ec_lod_loq$loq, 3)` [ug/sq.cm].

## Add a LOD and LOQ flag

```{r, echo=FALSE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}

  ecoc_merged <- dplyr::mutate(loq = NA, lod = NA) %>%
	
  ecoc_merged <- dplyr::filter(ecoc_merged, grepl("^[0-9]", id)) %>%
  ...
  lod_flags <- dplyr::select(ecoc_merged, date, id, cassette,
                                          qc, oc_lod, oc_loq,
                                          ec_lod, ec_loq)
```
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  ecoc_calcs <- dplyr::mutate(ecoc_merged, ec_ug = ec_ug_sq_cm * 11.79) %>%

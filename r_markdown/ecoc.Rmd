...
* F1 and F3 had bad (zero) calibration area
* L1.1 and L2 were analysed twice with and without He step, He step method retained.

```{r}
  ecoc <- ecoc %>%
          dplyr::filter((ecoc_id != "1/9/2017 B F1-A" | date != "0017-06-07") &
                        (ecoc_id != "1/9/2017 D F3-A" | date != "0017-06-07") &
                         ecoc_id != "L1.1-P" &
                         ecoc_id != "L2-P")
```
...
```{r}
  ecoc_merged <- ecoc_long %>%
                 ...
                 dplyr::select(id, id_test, conc, type, pol, dur, qc) %>%
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  ggplot(ecoc_merged, aes(id_test, value, color = pol)) +
  geom_point() +
  theme_bw() +
  scale_y_log10() +

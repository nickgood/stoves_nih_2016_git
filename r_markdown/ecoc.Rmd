...
```{r}
  df_ecoc <- dplyr::select(df_ecoc,
                           ...
                           punch_area_cm2, datetime, date, time, parameter_file)
```
...
```{r}
  df_flows <- dplyr::filter(flows, instrument == "filters") %>%
              ...
              dplyr::select(-colour, -loc, -instrument, -date)
```
...
Replace measurements below the LOD with LOD/sqrt(2). 

```{r}
  df_ecoc_merged <- dplyr::filter(df_ecoc_merged,
                                  (cassette == "qf") | (cassette == "qb"),
                                  substr(id,1,1) != "G")  %>%
                    dplyr::left_join(df_lod, by=c("pol")) %>%
                    dplyr::mutate(lod_flag = ifelse(ug_sq_cm >= lod, "above", "below"),
                                  ug_sq_cm = ifelse(lod_flag == "below", lod/sqrt(2), ug_sq_cm))
```

* The EC concentration on the front quartz filter was above the LOD for `r nrow(dplyr::filter(df_ecoc_merged, substr(id,1,2) != "BG", cassette == "qf", pol=="ec", lod_flag == "above"))`/`r nrow(dplyr::filter(df_ecoc_merged, substr(id,1,2) != "BG", cassette == "qf", pol=="ec"))` tests (background tests not included).  
* The OC concentration on the front quartz filter was above the LOD for `r nrow(dplyr::filter(df_ecoc_merged, substr(id,1,2) != "BG", cassette == "qf", pol=="oc", lod_flag == "above"))`/`r nrow(dplyr::filter(df_ecoc_merged, substr(id,1,2) != "BG", cassette == "qf", pol=="oc"))` tests (background tests not included).  
* The OC concentration on the back quartz filter was above the LOD for `r nrow(dplyr::filter(df_ecoc_merged, substr(id,1,2) != "BG", cassette == "qb", pol=="oc", lod_flag == "above"))`/`r nrow(dplyr::filter(df_ecoc_merged, substr(id,1,2) != "BG", cassette == "qb", pol=="oc"))` tests (background tests not included).  
 
* The EC concentration on the front quartz filter was above the LOD for `r nrow(dplyr::filter(df_ecoc_merged, substr(id,1,2) == "BG", cassette == "qf", pol=="ec", lod_flag == "above"))`/`r nrow(dplyr::filter(df_ecoc_merged, substr(id,1,2) == "BG", cassette == "qf", pol=="ec"))` background samples.  
* The OC concentration on the front quartz filter was above the LOD for `r nrow(dplyr::filter(df_ecoc_merged, substr(id,1,2) == "BG", cassette == "qf", pol=="oc", lod_flag == "above"))`/`r nrow(dplyr::filter(df_ecoc_merged, substr(id,1,2) == "BG", cassette == "qf", pol=="oc"))` background samples.  
* The OC concentration on the back quartz filter was above the LOD for `r nrow(dplyr::filter(df_ecoc_merged, substr(id,1,2) == "BG", cassette == "qb", pol=="oc", lod_flag == "above"))`/`r nrow(dplyr::filter(df_ecoc_merged, substr(id,1,2) == "BG", cassette == "qb", pol=="oc"))` background samples.  
...
```{r, fig.width=12, fig.height=8}
  ...
  ggplot(test_data, aes(id, ug_sq_cm, colour = lod_flag)) +
    geom_point(size=3) +
    theme_minimal() +
    theme(legend.position = "top") +

...
```{r, echo=FALSE, warning=FALSE, message=FALSE, results='markup', cache=FALSE, fig.width=12, , fig.height=8}
  ...
  ecoc_summary$stove_fuel <- factor(ecoc_summary$stove_fuel, levels = ecoc_summary$stove_fuel[order(ecoc_summary$id)])
  ...
  ggplot(ecoc_summary, aes(x = stove, y = ec_oc_ratio_avg, group = fuel, fill = fuel)) +   
    ...
    ylab("ec oc ratio") +
    xlab("") +
    ggtitle("ec oc ratios grouped by stove and fuel types") +

...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  ecoc_merged_2 <- dplyr::select(ecoc_merged, id, type, cassette, oc_ug_sq_cm, ec_ug_sq_cm, 
                               post_flow, pre_flow, end, start, qc, time)
```

## Remove duplicate analyses

```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  ecoc_merged_2$filter_id <- with(ecoc_merged_2, interaction(id,  cassette))
  ecoc_merged_2 <- dplyr::arrange(ecoc_merged_2, qc) %>%
                   dplyr::arrange(qc, desc(time))
  ecoc_merged_2 <- ecoc_merged_2[order(nrow(ecoc_merged_2):1),]
  ecoc_merged_2 <- ecoc_merged_2[!duplicated(ecoc_merged_2$filter_id),]
  ecoc_merged_2 <- dplyr::select(ecoc_merged_2, id, type, cassette, oc_ug_sq_cm,
                                 ec_ug_sq_cm, post_flow, pre_flow, end, start, qc) %>%
                   dplyr::arrange(id)
```
...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  ecoc_merged_2 <- dplyr::filter(ecoc_merged_2, !grepl("^G", id)) %>% #remove bkgds
                   dplyr::mutate(ec_ug = ec_ug_sq_cm * 11.8) %>%
                   dplyr::mutate(oc_ug = oc_ug_sq_cm * 11.8) %>%
                   dplyr::mutate(sample_time = end - start) %>%
                   dplyr::mutate(flow_avg = (post_flow + pre_flow)/2)
  #ecoc_merged_2 <- dplyr::mutate(ecoc_merged_2, ecoc_ratio = ec_ug_sq_cm / oc_ug_sq_cm)
```

## Save cleaner merged file

```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  save(ecoc_merged_2, file="../r_files/ecoc_merged_2.Rda")
```

## Remove blanks and backgrounds
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, results='markup', cache=FALSE, fig.width=12}
  ggplot(ecoc_merged_2, aes(id, ec_ug_sq_cm, colour = cassette)) +
         geom_point() +
         scale_y_log10() +
         theme_minimal() +
         ylab("ec ug/sq.cm") +
  ...
  ggplot(ecoc_merged_2, aes(id, oc_ug_sq_cm, color = cassette)) +
         geom_point() +
         scale_y_log10() +
         theme_minimal() +
         ylab("oc ug/sq.cm") +
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, results='markup', cache=FALSE, fig.width=12}
  ggplot(ecoc_merged_2, aes(id, ec_ug_sq_cm, colour = fuel)) +
         geom_point() +
         scale_y_log10() +
         theme_minimal() +
         ylab("ec ug/sq.cm")+
  ...
  ggplot(ecoc_merged_2, aes(id, oc_ug_sq_cm, color = fuel)) +
         geom_point() +
         scale_y_log10() +
         theme_minimal() +
         ylab("oc ug/sq.cm") +
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, results='markup', cache=FALSE, fig.width=12}
  ggplot(ecoc_merged_2, aes(id, ec_ug_sq_cm, color = qc)) +
         geom_point() +
         scale_y_log10() +
         theme_minimal() +
         ylab("ec ug/sq.cm")+
  ...
  ggplot(ecoc_merged_2, aes(id, oc_ug_sq_cm, color = qc)) +
         geom_point() +
         scale_y_log10() +
         theme_minimal() +
         ylab("oc ug/sq.cm") +

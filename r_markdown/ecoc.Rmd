...
** What is the aim here? I'd suggest leaving this step until the data is in an easier format to deal with.

```{r, echo=FALSE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}

  #ecoc_merged_$filter_id <- with(ecoc_merged, interaction(id,  cassette))

  #ecoc_merged <- dplyr::arrange(ecoc_merged, qc) %>%
  #                 dplyr::arrange(qc, desc(time))
  #ecoc_merged <- ecoc_merged[order(nrow(ecoc_merged):1),]
  #ecoc_merged <- ecoc_merged[!duplicated(ecoc_merged$filter_id),]
...
* redefine stove levels (no e moved to samples.Rda creation file)
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  ecoc_merged <- dplyr::filter(ecoc_merged, grepl("^[0-9]", id)) %>% #remove bkgds
                   ...
                   dplyr::mutate(ec_loq = ifelse(ec_ug_sq_cm > sd(blanks$ec_ug_sq_cm)*10,
                                       "ok","low"))
  lod_flags <- dplyr::select(ecoc_merged, date, id, cassette, qc, oc_lod, oc_loq, ec_lod, ec_loq)
```

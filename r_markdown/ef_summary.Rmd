...
```{r}
  df_emissions <- dplyr::filter(data_emissions, metric=="mass_carbon",
                                                ...
                                                substr(id,1,1) != "G",
                                                id != "FH2") %>%
                  dplyr::mutate(units = "g_carbon") %>%
                  dplyr::select(-inst, -metric) 
```
...
```{r}
  df_summary <- dplyr::bind_rows(df_ef_energy, df_ef_fuel) %>%
                ...
                dplyr::filter(pollutant %in% c("co2","co","ch4","grav","ec","oc",
                                 ...
                                 "2_5_dimethylbenzaldehyde",             "benzene", 
  ...
  df_summary <- dplyr::arrange(df_summary, units, pollutant, mc_level, fuel_shape)
```

...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE} 
  mass_fuel_p <- dplyr::left_join(mass_carbon, fuel_burnt, by = "id") %>%
...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  ...
  save(emissions, file = "../r_files/mass_carbon.Rda")
```
...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE, fig.width=10, eval=FALSE}  
  ggplot(mass_fuel_p, aes(x = mass_fuel_kg, y = mass_fuel_c, color = fuel)) +
    geom_point() +
    theme_minimal() +
    ylim(0, 2) +

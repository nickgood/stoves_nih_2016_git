...
```{r, fig.width = 10, fig.height = 6}
  p_data_ <- dplyr::filter(p_data, grepl("co2|co|ch4",pol), !grepl("FH2", id)) # comment this out since p_data redefined here
  ggplot(p_data_, aes(mc_level, mass_pol_mass_fuel, fill = pol), color = "black") +
  geom_boxplot(outlier.size = NA) +
  geom_point() +
  theme_bw() +
  scale_fill_brewer(palette = "PRGn") +
...
p_data_ <- dplyr::filter(p_data, pol %in% c("co", "co2", "ch4",
  ...
  geom_point(aes(color = Fuel_Shape), shape = 1) +
...
```{r}
  p_data_carb <- dplyr::filter(p_data, grepl("formaldehyde|acetaldehyde",pol), !grepl("FH2", id)) # comment this out since p_data redefined here

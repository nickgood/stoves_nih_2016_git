...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  emissions <- dplyr::left_join(emissions, 
                                dplyr::select(mass_fuel, id, mass_fuel_c, 
                                                         mass_fuel_kg), by = "id" ) %>%
               dplyr::mutate(mass_ef = ((mass_emitted * 1e-3) /
                                         mass_fuel$mass_fuel_kg),  # mg / kg
                             mass_c_ef = ((mass_emitted * 1e-3) /
                                         mass_fuel$mass_fuel_c),  # mg / kg
                             energy_ef = ((mass_ef * 1e-3) *
                                           mass_fuel$lhv * 1e-3), 
                             energy_c_ef = ((mass_c_ef * 1e-3) *
                                           mass_fuel$lhv * 1e-3))  # g / MJ
```
...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=TRUE, fig.width=10, fig.height=40, eval=TRUE} 
  ggplot(emissions, aes(x = id, y = mass_carbon, fill = inst), na.rm = TRUE) +
...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=TRUE, fig.width=10, fig.height=40, eval=TRUE} 
  ggplot(emissions, aes(x = id, y = mass_emitted, fill = inst), na.rm = TRUE) +
    geom_bar(stat = "identity")+
    coord_flip() +
    theme_minimal() +
    ylab("test id") + 

...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  mass_fuel <- dplyr::group_by(emissions, id) %>%
               dplyr::filter(mass_carbon >= 0) %>%  # ignore negatives
               dplyr::summarise(mass_carbon = sum(mass_carbon, na.rm = TRUE)) %>%
...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=TRUE, fig.width=10, fig.height=40} 
  ...
  emission_factors <- dplyr::filter(emissions, qc != "bad") %>%
...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  emission_factors <- dplyr::left_join(emission_factors, 
               ...
               dplyr::mutate(mass_ef = ((mass_emitted * 1e-3) / mass_fuel_kg),  # mg / kg
                             mass_c_ef = ((mass_emitted * 1e-3) / mass_fuel_c),  # mg / kg
...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  save(mass_fuel, file = "../r_files/mass_fuel.Rda")
  save(emission_factors, file = "../r_files/emission_factors.Rda")
```
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, results='markup', cache=FALSE, fig.width=12, fig.height=8}
  ggplot(dplyr::filter(emission_factors, pol == "co2"),
...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=TRUE, fig.width=10, fig.height=40, eval=TRUE} 
  ggplot(emission_factors, aes(x = id, y = mass_carbon, fill = inst), na.rm = TRUE) +
...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=TRUE, fig.width=10, fig.height=40, eval=TRUE} 
  ggplot(emission_factors, aes(x = id, y = mass_emitted, fill = inst), na.rm = TRUE) +
    geom_bar(stat = "identity")+
    coord_flip() +
    theme_minimal() +
    ylab("test id") + 

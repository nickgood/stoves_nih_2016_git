...
```{r}
  ...
  ecoc_ratio_fig <- ggplot(ecoc, aes(x = mean_mc, y = ratio, color = `MC Level`, shape = `Fuel Shape`)) +
  geom_point(size = 3.5) +
  theme_bw() +
  scale_color_manual(values = c("darkorange", "darkorchid3", "blue")) +
  theme(text = element_text(size = 15)) +
  ...
  ggsave(filename = "ecoc_ratio_fig.svg", plot = ecoc_ratio_fig, device = "svg", width = 8, height = 4, units = "in")
  #ggtitle("EC ratio")

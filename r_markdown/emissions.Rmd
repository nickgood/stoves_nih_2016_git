...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=TRUE}
  co_co2_ch4 <- dplyr::mutate(co_co2_ch4, mass_emitted = (conc - bg_conc) * flow_hood * (dur / 60))
```
...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=TRUE}
 emissions <- dplyr::select(co_co2_ch4, id, pol, mass_emitted) %>%
              dplyr::bind_rows(dplyr::select(carbonyls, id, pol, mass_emitted)) %>%
              dplyr::bind_rows(dplyr::select(ecoc, id, pol, mass_emitted)) %>%
              dplyr::bind_rows(dplyr::select(grav, id, pol, mass_emitted)) %>%
              dplyr::bind_rows(dplyr::select(ions, id, pol, mass_emitted)) %>%
              dplyr::bind_rows(dplyr::select(voc, id, pol, mass_emitted))
```

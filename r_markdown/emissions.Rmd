...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  ecoc <- dplyr::filter(ecoc_merged, type == "test" & qc == "ok") %>%
...
* replace NaN with between test mean values

```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  bg_co_ <- dplyr::mutate(bg_co,
 										 		ppm = ifelse(is.na(ppm), bg_co_mean$ppm[1], ppm),
 											 	conc = ifelse(is.na(conc), bg_co_mean$conc[1], conc))

  bg_co2_ <- dplyr::mutate(bg_co2,
 										 		ppm = ifelse(is.na(ppm), bg_co2_mean$ppm[1], ppm),
 											 	conc = ifelse(is.na(conc), bg_co2_mean$conc[1], conc))
 
  bg_ch4_ <- dplyr::mutate(bg_ch4,
 												ppm = ifelse(is.na(ppm), bg_ch4_mean$ppm[1], ppm),
 												conc = ifelse(is.na(conc), bg_ch4_mean$conc[1], conc))
```
...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=TRUE, fig.width=10, fig.height=40} 
  emissions_p <- dplyr::filter(emissions, grepl("^9B$|^6D$|^2A$|^8D", id) == FALSE) %>%
                 dplyr::mutate(mass_emitted_log = log10(mass_emitted))
```
...
# Bar plot of mass emitted by test

```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=TRUE, fig.width=10, fig.height=40} 
  ggplot(emissions_p, aes(x = id, y = mass_emitted_log, fill = pol), na.rm = TRUE) +
    geom_col() +
    coord_flip() +
    theme_minimal() +
    ylab("test id") + 
    ylab("log(mass emitted)") + 
    scale_y_log10() +
    theme(legend.position = "top")
```

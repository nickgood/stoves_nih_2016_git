...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  ...
  e_pot_water_mass <- dplyr::filter(batch_wgts,
                                    grepl("^on_[0-9]|^off_[0-9]", var)) %>%
                      tidyr::separate(var, c("var", "rep")) %>%
                      tidyr::spread(var, value) %>%
                      dplyr::select(-date) %>%
...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  e_temp <-  dplyr::select(temp, id, datetime, time, tamb, thood)
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, results='markup', cache=FALSE, fig.width=12, fig.height=80}
  ggplot(e_temp, aes(datetime, tamb, group = rep, colour = rep)) +
    geom_point() +
...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  e_temp <- dplyr::group_by(e_temp, id, rep) %>%
            dplyr::summarise(t_min = quantile(tamb, 0.05, na.rm = TRUE),
                             t_max = quantile(tamb, 0.95, na.rm = TRUE))
```
...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  e_data <- dplyr::left_join(e_times, e_pot_water_mass, by = c("id", "rep")) %>%
            ...
            dplyr::mutate(mass_pot = ifelse(pot == "a", a, NA),
                          ...
                          mass_pot = ifelse(pot == "c", c, mass_pot),
                          id = as.factor(id)) %>%
            dplyr::select(-a, -b)
```

...
Amount of fuel burnt during each test
...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
 # (N * 1,000,000 /cm^3) * m^3/min * (s / 60)
  ultrafine <- dplyr::mutate(ultrafine, emitted = number_conc * 1e6 * flow_hood * (dur/60))
```
...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  ...
  ions_g <- dplyr::group_by(ions, id) %>%
            dplyr::filter(!is.na(mass_emitted), !is.na(mass_carbon)) %>%
...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  emissions <- dplyr::select(co_co2_ch4, id, pol, mass_emitted,
               ...
               dplyr::bind_rows(dplyr::select(carbonyls, id, pol, mass_emitted,
                                                         mass_carbon, qc)) %>%
               dplyr::bind_rows(dplyr::select(carbonyls_g,
                                                         id, pol, mass_emitted,
                                                         mass_carbon, qc)) %>%
               ...
               dplyr::bind_rows(dplyr::select(ions, id, pol, mass_emitted,
                                                    mass_carbon, qc)) %>%
               dplyr::bind_rows(dplyr::select(ions_g, id, pol, mass_emitted,
                                                      mass_carbon, qc)) %>%
               dplyr::bind_rows(dplyr::select(voc, id, pol, mass_emitted,
                                                   mass_carbon, qc)) %>%
               dplyr::bind_rows(dplyr::select(voc_g, id, pol, mass_emitted,
                                                     mass_carbon, qc))
```

...
# filter out backgrounds and blanks
  ef_su_event <- dplyr::mutate(emission_factors, fuel = factor(fuel),
              ...
              dplyr::mutate(masspol_per_startup = (value * 1e-6) / fuel_quant, 
                            #value = ug emitted per test, so *1e-6 to convert to g for unit = g per event 
...
```{r}
  ef_sum <- ef_su_event %>% dplyr::group_by(fuel, pol) %>%
                            dplyr::summarize(nreps = n(),
                                     mean_ef = round(mean(masspol_per_startup, na.rm = TRUE), 5),
                                     min_ef = round(min(masspol_per_startup, na.rm = TRUE), 5),
                                     max_ef = round(max(masspol_per_startup, na.rm = TRUE), 5))

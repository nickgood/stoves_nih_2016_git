...
# stacked btex data
  ...
  btex <- ef_su_event %>%
          ...
          dplyr::summarize(mean_ef = mean(mass_avg_startup, na.rm = TRUE)) %>%
          dplyr::ungroup()
...
# stacked carbs data
  carbs <- ef_su_event %>% 
          ...
          dplyr::group_by(pol, fuel) %>%
          dplyr::summarize(mean_ef = mean(mass_avg_startup, na.rm = TRUE)) %>% #this isn't working by group- it's giving the same sum for each pollutant...
...
# stacked carcinogenic data
  cancer <- ef_su_event %>% 
          ...
          dplyr::group_by(pol, fuel) %>%
          dplyr::summarize(mean_ef = mean(mass_avg_startup, na.rm = TRUE)) %>%#this isn't working by group- it's giving the same sum for each pollutant- partially (sometimes its a split between the voc and carbs?)...

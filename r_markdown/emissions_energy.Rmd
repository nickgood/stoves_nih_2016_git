...
* temperature change v2.0

```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  t_range_2 <- dplyr::left_join(e_times, e_temp, by = c("id" = "id", "rep" = "rep")) %>%
               dplyr::mutate(dt_start = abs(start - time), dt_end = abs(end - time)) %>%
               dplyr::group_by(id, rep, loc) %>%
               dplyr::arrange(dt_start) %>%
               dplyr::filter(dt_start %in% range(dt_start))
```
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, results='hide'}
  p_temp <- dplyr::distinct(e_data) %>%
            dplyr::select(id, rep, t_min, t_max, dt) %>%
            dplyr::group_by_(.dots = c("id", "rep")) %>% 
            #tidyr::gather("t_var", "val", 3:ncol(.)) %>%
            dplyr::mutate(id_rep = paste(id, "; rep", rep)) %>%
            dplyr::left_join(dplyr::select(dplyr::filter(samples, type == "SF"),
                                           id, stove, fuel, fuelcat), energy, by = "id")
```

```{r, echo=FALSE, warning=FALSE, message=FALSE, results='markup', cache=FALSE, fig.width=12, fig.height=20}
  ggplot(p_temp, aes(id_rep, ymin = t_min, ymax = t_max , colour = fuel)) +
    geom_linerange(width = 1.2) +
    facet_wrap(~ fuelcat, ncol = 1, scales = 'free') +
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 60, hjust = 1)) +
    theme(legend.position = "top") +
    ylab("temp (oC)") +

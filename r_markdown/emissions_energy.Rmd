...
t_range1 <- dplyr::group_by(e_temp1, id) %>%
            dplyr::summarise(t_min = min(t_oc, na.rm = TRUE),  
                             t_max = max(t_oc, na.rm = TRUE),
                             t_med = median(t_oc),
                             t_high_avg = t_med + 0.5*(t_max - t_med))
...
mean_mc <- readRDS("../r_files/mean_mc_df.RDS")
 ...
 ggplot(data = temp_plot1, aes(x = mean_mc)) +
...
ggplot(all_energy, aes(x = mean_mc, y = mass_pol_energy_mg, shape = type)) + #color = mc_level, 
  geom_point(size = 2) +
  theme_bw() +
  ...
  scale_x_continuous(breaks = c(5, 15, 25)) +
  labs(color = "MC Level", shape = "Fuel Shape") +
  geom_hline(aes(yintercept = 979), size = 1, data=subset(all_energy, pol == "grav"), color = "#FF3366") +
  geom_hline(aes(yintercept = 386), size = 1,data=subset(all_energy, pol == "grav"), color = "#330099") +
  geom_hline(aes(yintercept = 168), size = 1,data=subset(all_energy, pol == "grav"), color = "yellow") +
  geom_hline(aes(yintercept = 41), size = 1,data=subset(all_energy, pol == "grav"), color = "green") +
  geom_hline(aes(yintercept = 16000), size = 1,data=subset(all_energy, pol == "co"), color = "red") +
  geom_hline(aes(yintercept = 11000), size = 1,data=subset(all_energy, pol == "co"), color = "orange") +
  geom_hline(aes(yintercept = 9000), size = 1,data=subset(all_energy, pol == "co"), color = "yellow") +
  geom_hline(aes(yintercept = 8000), size = 1,data=subset(all_energy, pol == "co"), color = "green") +
  facet_wrap(~pol, ncol = 4, scales = "free_y") + #dummy value for now to fix panel issue
  geom_text(aes(label = "CO", x = 25, y = 3800), colour = "black", data=subset(all_energy, pol == "co")) +
  geom_text(aes(label = "PM2.5", x = 25, y = 150), colour = "black", data=subset(all_energy, pol == "grav")) +

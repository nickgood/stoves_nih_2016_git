...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='hide'}
 # time tweak
  ...
  temp_tweak <- dplyr::left_join(temp_merged, time_tweak, by = "id") %>%
...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  temp_tweak <- dplyr::filter(temp_tweak, loc != "flue", id == "16A")

  temp_tweak <- filter_temp(e_times, temp_tweak)
```

```{r, echo=FALSE, warning=FALSE, message=FALSE, results='markup', cache=FALSE, fig.width=5, fig.height=5}
  p_data <- dplyr::filter(temp_tweak, id == "16A")

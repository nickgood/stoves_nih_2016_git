...
```{r}
  df_ef <- dplyr::left_join(df_ef,
           ...
           dplyr::mutate(id = forcats::fct_relevel(id, "FL1", "FL2", "FL3", "FL4",
                                                       ...
                                                       "LH1", "LH2", "LH3","LH4"),
                         mc_target = 25,
                         mc_target = ifelse(mc_level == "Medium", 15, mc_target),
                         mc_target = ifelse(mc_level == "Low",     5, mc_target)) %>%
...
```{r, echo=FALSE, fig.width=7.48, fig.height=3.54, fig.align='center'}
  ...
  ggplot(df_ef_subset, aes(x=mc_dry, y=1000*ef, shape=type, color=pol)) +
    ...
    geom_hline(aes(yintercept=   5), size=0.5, data=subset(df_ef_subset, pol=="grav"), linetype=2, color="gray75") +
      ...
      geom_text(aes(label="Acetaldehyde", x= 4, y=  110), data=subset(df_ef_subset, pol=="acetaldehyde"),
                color="black", hjust="left", vjust="top", size=3.5) +
      geom_text(aes(label="Benzene",      x= 4, y=   70), data=subset(df_ef_subset, pol=="benzene"),
                color="black", hjust="left", vjust="top", size=3.5) +
...
```{r, echo=FALSE, fig.width=6.5, fig.height=6.7, fig.align='center'}
  ...
  ggsave(filename="../figures/carbonyls_energy.pdf", plot=last_plot(), device="pdf", width=6.5, height=6.7)
```
...
```{r}
  ...
  for(i in 1:length(pol_list)){
    ...
    df_i  <- dplyr::filter(df_ef, pol==pol_i)
    
    # one-way ANOVA
    aov_i <- aov(ef ~ mc_level, df_i)
    ...
    df_i <- data.frame(pol_i) %>%
            dplyr::mutate(pol_i   = as.character(pol_i),
                          p_ftest = summary(aov_i)[[1]]$`Pr(>F)`[1],
...
```{r}
  ...
  for(i in 1:length(pol_list_log)){
    ...
    df_i  <- dplyr::filter(df_ef_anova, pol==pol_i)
    
    # one-way ANOVA
    aov_i <- aov(log_ef ~ mc_level, df_i)
    ...
    df_i <- data.frame(pol_i) %>%
            dplyr::mutate(pol_i   = as.character(pol_i),
                          p_ftest_log = summary(aov_i)[[1]]$`Pr(>F)`[1],
...
```{r}
  ...
  for(i in 1:length(pol_list)){
    ...
    # Model 1: Is there any association between MC and EF?
    fit1 <- lm(ef ~ mc_level, df_i)
    
    f1 <- summary(fit1)$fstatistic
    
    p1 <- pf(f1[1], f1[2], f1[3], lower.tail=FALSE)
    
    # Model 2: Does a linear model have a nonzero slope?
    fit2 <- lm(ef ~ mc_target, df_i)
    
    f2 <- summary(fit2)$fstatistic
    
    p2 <- pf(f2[1], f2[2], f2[3], lower.tail=FALSE)
    
    # Are models 1 and 2 equivalent, or is the relationship
    # between MC and EF nonlinear? 
    anova_i <- anova(fit1, fit2)
    ...
    if(i==1){
      ...
      df_linear <- dplyr::bind_rows(df_linear,df_i)
    }
  }
```

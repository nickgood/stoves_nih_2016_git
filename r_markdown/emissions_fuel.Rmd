...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
fuel_burnt_reps <- dplyr::left_join(mass_fuel, times_fuel, by=c("id", "rep", "id_test", "date", "type")) %>%
                   dplyr::left_join(test_info, fuel_burnt_reps, by = c("id", "id_test", "date"))
```
...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  fuel_burnt_test <- dplyr::group_by(fuel_burnt_reps, id) %>% summarise(
                     ...
                     dplyr::left_join(test_info, fuel_burnt_test, by="id") %>%
                     dplyr::left_join(times_test, fuel_burnt_test, by=c("id", "id_test", "date"))
```

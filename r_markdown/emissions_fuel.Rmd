...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  fuel_burnt_reps <- times_fuel %>% 
                     ...
                     left_join(mass_fuel, by = c("id", "id_test", "date", "type", "rep"))
```
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, results='markup', cache=FALSE, fig.width=12, fig.height=8}
  ggplot(filter(fuel_burnt_reps, fuel != "blank"), aes(x = rep, y = dm, colour = fuel)) +
        geom_point() +
        theme_minimal() + 
        facet_wrap(~ id_test) +
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, results='markup', cache=FALSE, fig.width=12, fig.height=8}
  ggplot(filter(fuel_burnt_test, fuel != "blank"), 

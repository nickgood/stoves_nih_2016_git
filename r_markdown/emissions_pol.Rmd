...
#voc <- readRDS("../r_files/voc_merged.RDS") #currently not complete data/not merging IDs correctly
...
```{r}
fivegas_full <- readRDS("../r_files/fivegas_full.RDS")
fivegas_bg <- readRDS("../r_files/fivegas_background.RDS")
fivegas_byrep <- readRDS("../r_files/fivegas_byrep.RDS")
...
# grav - FIRST ATTEMPT DONE as of 6/27/17; note question on calibration adjustments and negative emissions
...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  grav <- dplyr::mutate(grav, dur = (end - start) / 60,
                              conc_noadj = delta * 1000 / (flow_mean * dur), 
                              qc = as.factor(qc),
                              conc = conc)
```
NOTE: conc_noadj is different than the conc calculated within the grav.Rmd (conc), because in grav.Rmd we adjusted for the calibration weight and the blanks.
...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  head(grav, 2)
```
NOTE: why are there negative numbers? 

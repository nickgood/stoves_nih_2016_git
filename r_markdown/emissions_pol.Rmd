...
```{r}
  df_mce <- dplyr::select(co_co2_ch4_ppm, id, pol, ppm, ppm_bg) %>%
            ...
            dplyr::left_join(select(samples, id, type, mc_level), by=c("id")) %>%
            dplyr::left_join(mean_mc, by = "id")                              %>%
            dplyr::mutate(type = forcats::fct_recode(type, "Split"  = "field",
                          ...
                          mc_level = forcats::fct_relevel(mc_level, "low",
                                                                    ...
                                                                    "high")) %>%
            # Filter data
            dplyr::filter(type != "bg", !is.na(mean_mc)) %>%
...
```{r, fig.width=3.15, fig.height=3.15, fig.align='center'}
  ...
  ggplot(data=df_mce, aes(x=mean_mc, y=100*mce, shape=type)) + 
  ...
  saveRDS(df_mce, file = "../r_files/mce.RDS")
...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  ...
  df_temp_e  <- dplyr::filter(temp_merged, loc=="pot") %>%
...
I updated this section of code on Dec. 6, 2018. -- J.T. 

```{r}

  # constants needed to calculate firepower
  lhv    <- 19320 # kJ/kg; lower heating value of Douglas Fir on a dry basis
  y_c    <- 0.5   # kg carbon/kg fuel

  # calculate firepower
  df_fp <- dplyr::select(co_co2_ch4, id, pol, mass_carbon, dur) %>%
           tidyr::spread(pol, mass_carbon) %>%
           dplyr::mutate(m_carbon  = co2 + co + ch4, # ug
                         firepower = (m_carbon*lhv)/(y_c*dur*(10^9))) %>% # kW
           dplyr::select(-co2,-co,-ch4) %>%
           dplyr::left_join(select(samples, id, type, mc_level), by=c("id")) %>%
           dplyr::left_join(mean_mc, by = "id") %>%
           dplyr::mutate(type = forcats::fct_recode(type, "Split"  = "field",
...
```{r, fig.width=3.15, fig.height=3.15, fig.align='center'}
  ggplot(df_fp, aes(x=mean_mc, y=firepower, shape=type)) +
    geom_point(size=3, alpha=0.7) +
  ...
  ggsave(filename="firepower.pdf", plot=last_plot(), device="pdf", width=8/2.54, height=8/2.54)
```

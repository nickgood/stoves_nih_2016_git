...
```{r}
  ...
  fivegas_bg_mean <- readRDS("../r_files/fivegas_background_mean.RDS")
...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  co_co2_ch4 <- dplyr::left_join(co_co2_ch4, 
                dplyr::select(fivegas_bg_mean, id, pol, bg_ppm), by = c("id", "pol")) %>%
...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  emissions_long <- tidyr::gather(emissions, "metric", "value", 3:5)
```

...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  ...
  co_co2_ch4_ <- dplyr::left_join(co_co2_ch4, 
                dplyr::select(fivegas_background, id, pol, bg_ppm), by = c("id", "pol"))

  bg_missing <- dplyr::filter(co_co2_ch4_, is.na(bg_ppm)) %>%
                dplyr::select(-bg_ppm) %>%
                dplyr::left_join(dplyr::select(fivegas_background_mean, pol, bg_ppm),
                                 by = "pol")

  co_co2_ch4_ <- dplyr::bind_rows(dplyr::filter(co_co2_ch4_, !is.na(bg_ppm)),
                                 bg_missing) %>%

...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  bg_mean <- dplyr::group_by(out_bg, id, type, pol) %>%
...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  ...
  flags <- dplyr::select(notes, id, qc) %>%
           ...
           dplyr::mutate(qc = ifelse(is.na(qc), "good", qc))
```


```

  out_full_ <- dplyr::left_join(out_full, 
  ...
  out_rep <- dplyr::left_join(out_rep,
                              dplyr::select(bg_mean, id, pol, bg_ppm),
                              by = c("id", "pol")) %>%
...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  ...
  saveRDS(bg_mean, file = "../r_files/fivegas_background_mean.RDS")
```

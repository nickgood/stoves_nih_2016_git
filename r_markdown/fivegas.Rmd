...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  fivegas_conc <- tibble::as_tibble(dplyr::select(fivegas_conc, -datetime_secs, -time_str) %>%
...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='hide', cache=FALSE}
  fivegas_calibration <- as_tibble(tidyr::gather(fivegas_calibration, pol, volts, co2:ch4))
```
...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  fivegas_cal <- tidyr::spread(fivegas_cal_times, type, value) %>%
                 ...
                 dplyr::rowwise() %>%
                 dplyr::mutate(v_mean = time_window_stats(fivegas_calibration, date,
                                                          start, end,
                                                          pol_var = pol, val_var = "volts",
                                                          stat_var = "mean")) %>%
                 dplyr::mutate(v_sd = time_window_stats(fivegas_calibration, date,
                                                        start, end,
                                                        pol_var = pol, val_var = "volts",
                                                        stat_var = "sd"))
```
...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  fivegas_cal <- dplyr::mutate(fivegas_cal,
                        v_zero_mean = time_window_stats(fivegas_calibration,
                                                        date, start_zero, end_zero,
                                                        pol_var = pol, val_var = "volts",
                                                        stat_var = "mean")) %>%
                 dplyr::mutate(v_zero_sd = time_window_stats(fivegas_calibration,
                                                             date, start_zero, end_zero,
                                                             pol_var = pol, val_var = "volts",
                                                             stat_var = "sd"))
```
...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  ...
  fivegas_cal <- dplyr::left_join(fivegas_cal, fivegas_cal_conc, by = c("date", "pol"))
```
...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  fivegas_cal <- dplyr::mutate(fivegas_cal, cal_beta = (standard / (v_mean - v_zero_mean))) %>%
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, results='markup', cache=FALSE, fig.width=12, fig.height=6}
 p_df <- tidyr::gather(fivegas_cal, "type", "volts", v_mean, v_zero_mean) %>%
...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  fivegas_volts <- dplyr::select(fivegas_volts, datetime, date, time, id, pol, ppm)
```
...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  fivegas_merged <- dplyr::left_join(fivegas_merged, flags, by = "id") %>%
                    dplyr::mutate(id = as.factor(id),
                                  qc = as.factor(ifelse(is.na(qc), "ok", as.character(qc))))
...
NG: Calibration files for 2016/03/09 and 2016/04/01 missing?

Plot QC

```{r, echo=FALSE, warning=FALSE, message=FALSE, results='markup', cache=FALSE, fig.width=12, fig.height=80, eval=FALSE}
  df <- dplyr::filter(fivegas_merged, !is.na(ppm) & pol == "co")  # filter
  ...
  df <- dplyr::filter(fivegas_merged, !is.na(ppm) & pol == "co2")  # filter
  ggplot(df,  aes(datetime, ppm, colour = qc, group = pol)) +
        geom_line() +
        scale_y_log10(limits = c(1, 10^5)) +
        facet_wrap(~id, ncol = 3, scale = "free") +
  ...
  df <- dplyr::filter(fivegas_merged, !is.na(ppm) & pol == "ch4")  # filter
  ggplot(df,  aes(datetime, ppm, colour = qc, group = pol)) +
        geom_line() +
        scale_y_log10(limits = c(1, 10^5)) +
        facet_wrap(~id, ncol = 3, scale = "free") +
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, results='markup', cache=FALSE, fig.width=12, fig.height=40, eval=FALSE}
  df <- dplyr::filter(fivegas_merged, !is.na(ppm) & pol != "o2" & pol != "nox") %>%
        dplyr::mutate(id = as.character(id)) %>%
        dplyr::arrange(id) %>%
        dplyr::mutate(id = as.factor(id))

  ggplot(df, aes(datetime, ppm, colour = pol)) + 
        geom_line() +
        scale_y_log10(limits = c(1, 10^5)) +
        facet_wrap(~id, ncol = 3, scale = "free") +
        theme_minimal()

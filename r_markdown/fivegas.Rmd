...
For CO and CO2, all tests follow expected "replicate" pattern of increasing and then falling levels as bunches of material are burned. Levels appear within expected ranges.The CH4 looks a little odd and doesn't necessarily follow this pattern, however, when grouped by material type we see the same patterns across the same materials so it's likely that the pattern and emission levels are at a scale that is just hard to follow on these plots. Tests 31-35 are "flat", which is as expected as these are background tests where no burning occured.The background 

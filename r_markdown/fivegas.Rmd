...
###CH4 only: 
```{r, echo=FALSE, warning=FALSE, message=FALSE, results='markup', cache=FALSE, fig.width=12, fig.height=40, eval=TRUE}
  ggplot(filter(out_full, pol == "ch4"), aes(datetime, val), group_by()) + 
    geom_point() +
    scale_y_log10() +
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, results='markup', cache=FALSE, fig.width=12, fig.height=40, eval=TRUE}
gas_sum <- plyr::ddply(out_full, c("pol"), summarise, 
...
knitr::kable(gas_sum, format = "markdown", col.names=c("pollutant","mean value (ppm)", " value median (ppm)", "max value (ppm)", "minimum value (ppm)", "standard error"))
```

...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  background_test_mean <- dplyr::group_by(out_bg, id, type, pol) %>%
                          dplyr::summarise(mean = mean(val, na.rm = TRUE)) %>%
                          tidyr::spread(type, mean) %>%
                          mutate(delta = post - pre,
                                 bg_ppm = (pre + post) / 2, # pre/post mean
                                 bg_ppm = ifelse(is.na(bg_ppm), pre, bg_ppm), # try pre only
                                 bg_ppm = ifelse(is.na(bg_ppm), post, bg_ppm)) # try post only
```

# QC

```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  notes <- dplyr::filter(notes, grepl("fivegas|all", inst))
...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  ...
  out_full <- out_full %>%
              dplyr::left_join(out_full, flags, by = "id") %>%
...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  ...
  saveRDS(background_test_mean, file = "../r_files/fivegas_background_mean.RDS")
```
...
gas_sum_tests <- plyr::ddply(filter(out_full, fuel != "background"), c("pol"), summarise, 
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, results='markup', cache=FALSE, fig.width=12, fig.height=40, eval=TRUE}
gas_sum_bg <- plyr::ddply(filter(out_full, fuel == "background"), c("pol"), summarise, 
                   meanval = round(mean(val, na.rm = TRUE), 2),
                   medianval = median(val, na.rm = TRUE),
                   maxval = max(val, na.rm = TRUE),
                   minval = min(val, na.rm = TRUE),
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, results='markup', cache=FALSE, fig.width=12, fig.height=40, eval=TRUE}
  ggplot(filter(out_full, pol == "ch4"), aes(datetime, val, color = fuel)) + 
    geom_point() +
    scale_y_log10() +
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, results='markup', cache=FALSE, fig.width=12, fig.height=40, eval=TRUE}
  ggplot(filter(out_rep, pol == "ch4"), aes(datetime, val, color = fuel)) + 
    geom_point() +
    scale_y_log10() +
    facet_wrap(~id, ncol = 4, scales = "free_x") +
    theme_minimal() +
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, results='markup', cache=FALSE, fig.width=10, fig.height=20, eval=TRUE}
 df <- filter(out_rep, pol == "ch4") %>% group_by(fuel, id) %>% mutate(x = seq_along(val)) %>% ungroup()

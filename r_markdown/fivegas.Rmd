...
mce <- dplyr::mutate(mce, mc_level = forcats::fct_relevel(mc_level, "low", "med", "high")) %>%
       dplyr::mutate(mc_level = forcats::fct_recode(mc_level, "medium" = "med")) %>%
       dplyr::group_by(mc_level, type)


ggplot(data = mce, aes(x = mc_level, y = mce, fill = mc_level), color = "black") +
  geom_boxplot(size = 1) +
  geom_point() +
  theme_bw() +
  labs(x = "Moisture Content of Wood", y = "Modified Combustion Efficiency (%)") +

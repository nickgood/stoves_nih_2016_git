...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  ...
  grav_merged <- dplyr::mutate(grav_merged,
                               type = ifelse(grepl("^[0-9]", id) == TRUE, "test", "")) %>%
                 dplyr::mutate(type = ifelse(grepl("^G", id) == TRUE, "bg", type)) %>%
...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  bg <- dplyr::filter(grav_merged, type == "bg", qc == "ok") %>%
        dplyr::select(id,
                         ...
                         time_end, time_start,
                         wgt_delta) %>%
        dplyr::mutate(flow_pre = ifelse(is.na(flow_pre) == TRUE, flow_post,
                                                                 flow_pre)) %>%
        na.omit() %>%
        dplyr::mutate(flow = (flow_pre + flow_post) / 2,
...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
 # extract data for merge
  bg <- dplyr::select(bg, type, mean) %>%
        dplyr::rename(conc_bg = mean)

 # merge and add pollutant variable
  grav_merged <- dplyr::left_join(grav_merged, bg, by = "type") %>%
                 dplyr::mutate(pol = as.factor("grav"))
```

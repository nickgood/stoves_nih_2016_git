...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  df_grav_merged <- dplyr::mutate(df_grav_merged,
                                  lod = ifelse(d_mass >= lod_var, "above", "below"),
...
3. Calculate background concentration.  

```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  df_bg <- dplyr::filter(df_grav_merged, is.na(test_type)) %>% # 1
           dplyr::select(id, flow, start_filters, end_filters, d_mass, lod, loq) %>%
           na.omit() %>% # 2
...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  df_grav_merged <- dplyr::mutate(df_grav_merged, conc_bg = bg_summary$mean)
```

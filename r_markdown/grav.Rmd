...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  grav_merged <-# dplyr::filter(grav, !grepl("^G[0-9]", id)) %>%
                 dplyr::select(grav,-lod,-phpa_pre,-phpa_post) %>%
                 dplyr::mutate(type=ifelse(grepl("^BG[0-9]", id) == TRUE, "bg", ifelse(grepl("^G[0-9]",id)==TRUE,"test_blank","test")))
```

## set levels

```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  levels_type <- c("test", "bg","test_blank")
```
...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  grav_merged<- dplyr::mutate(grav_merged,test_type=ifelse(grepl("^G[1-9]",id)== TRUE,"test_blank",
                                                          ifelse(grepl("^BG[1-9]",id)==TRUE, "background",
                                                          ifelse(grepl("^F[A-Z][1-9]",id)==TRUE, "field","lab"))))
...
# LOD/LOQ DID NOT FIX G VS LAB BLANK ISSUE YET 7/12/17

* remove backgrounds and bad data then calculate the limit of detection and quantification

```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  signal <- dplyr::filter(grav_merged, type == "test_blank") %>% #, qc != "bad") %>%
...
## weights REMOVE COLOR=QC OR SET DIFF COLOR

```{r, echo=FALSE, warning=FALSE, message=FALSE, results='markup', cache=FALSE, fig.width=12, fig.height=8}
  ggplot(dplyr::filter(grav_merged, !grepl("^G[0-9][A-Z]", id)==TRUE),
         aes(id, wgt_delta)) +
  geom_point() +
  theme_minimal()
  
  # ggplot(dplyr::filter(grav_merged, grepl("^[1][0-9][A-Z]", id)==TRUE),
  #        aes(id, wgt_delta, color= 'blue')) +
  # geom_point() +
  # theme_minimal()
  # 
  # ggplot(dplyr::filter(grav_merged, grepl("^[2-3][0-9][A-Z]", id)==TRUE),
  #        aes(id, wgt_delta, color= 'green')) +
  # geom_point() +
  # theme_minimal()
  
  #Edited the below section 7/12 to capture lab blanks from original grav file
  
  ggplot(dplyr::filter(grav, grepl("^G[1-9]", id)==TRUE),
         aes(id, wgt_dif)) +
  geom_point() +
  theme_minimal()
```

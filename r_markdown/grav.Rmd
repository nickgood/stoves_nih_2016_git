...
```{r}
  grav_merged <-# dplyr::filter(grav, !grepl("^G[0-9]", id)) %>%
                 ...
                 dplyr::mutate(type=ifelse(grepl("^BG[0-9]", id), "bg",
                                           ifelse(grepl("^G[0-9]",id), "test_blank","test")))
```
...
```{r}
  blanks <- grav_merged %>%
            dplyr::filter(type == "test_blank")
```

* remove blanks from grav variable
...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  grav_merged <- grav_merged %>%
                 ...
                 dplyr::mutate(conc_bg = bg$conc_bg)
```

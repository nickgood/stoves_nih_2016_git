...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  grav_merged <- dplyr::left_join(grav_merged, select(notes, id, qc), by = "id")
  grav_merged$id <- as.factor(grav_merged$id)
...
$Gravimetric$ data was collected during `r length(unique(grav_merged$id))` experiments between `r min(grav_merged$date, na.rm = TRUE)` and `r max(grav_merged$date, na.rm = TRUE)`. There is no $Gravimetric$ data for tests: `r setdiff(levels(samples$id), levels(grav_merged$id))`.

...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
 grav <- dplyr::left_join(grav, dplyr::select(times_test, id, start, end))
```
...
B[0-9] are blank filters. BG[0-9] are background tests.
...
* remove filter that has id = NA (an excess blank)
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  grav <- dplyr::filter(grav, id !='NA')
```
# Summary
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, results='markup', cache=FALSE, fig.width=12, fig.height=8}
  ggplot(grav) +
    geom_point(aes(id_test, conc, colour = type)) +
    geom_point(aes(id_test, bg_conc), colour = "black") +
    theme_bw() +
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, results='markup', cache=FALSE, fig.width=20, fig.height=15}
  ggplot(filter(grav, fuel != "background"), aes(id, conc_bgadj, colour = qc)) +
  geom_point(size = 4) +
  theme_minimal() +

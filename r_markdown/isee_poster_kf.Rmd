...
```{r}
 avg <- emissions_factors %>%
        ...
        dplyr::summarise(mean = round(mean(energy_ef_comb, na.rm = TRUE), 1))
```
...
```{r}
  ...
  plot_ef <- ggplot(emissions_factors, aes(y = energy_ef_comb, x = stovecat)) +
         geom_point(position = position_dodge(0.5), size = 2, aes(group = fuel, color = fuel)) +
         geom_point(aes(y = mean, x = stovecat, group = fuelcat), data = avg, position = position_dodge(0.5), shape = 8, size = 4) +
...
```{r}
 # r2 for all fuels combined
  ...
  PMbenz_plot <- ggplot(ef_plotdata, aes(x = PM2.5, y = benzene)) + 
      ...
      scale_color_hue(l = 55) +
      geom_smooth(method = "lm", se = FALSE, aes(group = fuelcat, color = fuelcat)) +
      geom_text(data = r2_data, aes(x = 5000, y = 2700, label = eq, family = "Helvetica Neue"),
                      color = 'black',  parse = TRUE,
                      size = 8) +
      xlab("PM2.5 EF (mg/kg of fuel)") +
...
```{r}
 # r2 for all fuels combined
  r2_data <- data.frame(eq = unclass(lm_mod(ef_plotdata, "benzene ~ CO")))

  CObenz_plot <- ggplot(ef_plotdata, aes(x = CO, y = benzene)) + 
      geom_point(size = 5, stroke = 1.5, aes(color = fuelcat, shape = stovecat)) +
      scale_color_hue(l = 55) +
      geom_smooth(method = "lm", se = FALSE, aes(group = fuelcat, color = fuelcat)) +
      geom_text(data = r2_data, aes(x = 2.3e5, y = 2700, label = eq, family = "Helvetica Neue"),
                      color = 'black',  parse = TRUE,
                      size = 8) +
      ylab("Benzene EF (mg/kg of fuel)") +
...
```{r}
 # r2 for all fuels combined
  ...
  PMform_plot <- ggplot(ef_plotdata, aes(x = PM2.5, y = formaldehyde)) + 
      ...
      scale_color_hue(l = 55) +
      geom_smooth(method = "lm", se = FALSE, aes(group = fuelcat, color = fuelcat)) +
      geom_text(data = r2_data, aes(x = 5000, y = 1400, label = eq, family = "Helvetica Neue"),
                      color = 'black',  parse = TRUE,
                      size = 8) +
      xlab("PM2.5 EF (mg/kg of fuel)") +
...
```{r}
 # r2 for all fuels combined
  r2_data <- data.frame(eq = unclass(lm_mod(ef_plotdata, "formaldehyde ~ CO")))

  COform_plot <- ggplot(ef_plotdata, aes(x = CO, y = formaldehyde)) + 
      geom_point(size = 5, stroke = 1.5, aes(color = fuelcat, shape = stovecat)) +
      scale_color_hue(l = 55) +
      geom_smooth(method = "lm", se = FALSE, aes(group = fuelcat, color = fuelcat)) +
      geom_text(data = r2_data, aes(x = 2.0e5, y = 1400, label = eq, family = "Helvetica Neue"),
                      color = 'black',  parse = TRUE,
                      size = 8) +
      ylab("Formaldehyde EF (mg/kg of fuel)") +
      xlab("CO EF (mg/kg of fuel)") +

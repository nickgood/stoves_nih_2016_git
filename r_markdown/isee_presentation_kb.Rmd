...
```{r global_options, include=FALSE}
  knitr::opts_chunk$set(fig.path='figures/', fig.height = 6, fig.width = 8, warning=FALSE, message=FALSE, cache=FALSE)
```
...
```{r}
  ...
  btex <-
    emissions %>% 
    dplyr::filter(grepl("^voc_benzene$|^voc_toluene$|^voc_ethylbenzene$|xylene", pol)) %>%
    dplyr::group_by(id) %>%
    dplyr::summarise_if(is.numeric, sum) %>%

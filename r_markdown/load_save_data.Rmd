...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  samples <- dplyr::mutate(samples,
                           stove = factor(stove, levels = c("Three Stone Fire (Artisan)",
                                                            "Clay Ring (Artisan)",
...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  samples <- dplyr::mutate(samples, fuel = factor(fuel, levels = c("Douglas Fir",
                                                                   "Oak",
                                                                   "Eucalyptus",
                                                                   "Lump Hardwood (Sm.)",
                                                                   "Lump Hardwood (Med.)",
...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  samples <- dplyr::mutate(samples, fuelcat = fct_collapse(fuel,
                                              wood = c("Oak",
                                                       "Eucalyptus",
                                                       "Douglas Fir"),
...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  samples <- dplyr::mutate(samples, fuelcat = factor(fuelcat,
                                              levels(fuelcat)[c(3,2,4,5,1)])) %>%
             dplyr::mutate(stovecat = fct_collapse(stove,
                                      ...
                                      advanced = c("Pressurized LPG (WokSmith, WS-C1-25K)",
                                                   "Wick Kerosene (Cook 'n Lite, 82)", 
                                                   "Pressurized Kerosene (Butterfly, 2412)"))) %>%
            dplyr::mutate(stovecat = factor(stovecat,levels(stovecat)[c(3,2,4,5,1)]))
```

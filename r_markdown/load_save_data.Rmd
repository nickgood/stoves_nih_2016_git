...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  samples <- dplyr::mutate(samples, stove =
                           lvls_revalue(stove, c("Three Stone Fire (Artisan)",
                             "Clay Ring (Artisan)",
...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  samples <- dplyr::mutate(samples,
                           fuelcat = lvls_reorder(fuelcat, c(1,2,3,4,5))) %>%  # set factor level order for plots
             dplyr::mutate(stovecat =
                           fct_collapse(stove,
                             ...
                             advanced = c("Pressurized LPG (WokSmith, WS-C1-25K)",
                                          "Wick Kerosene (Cook 'n Lite, 82)", 
                                          "Pressurized Kerosene (Butterfly, 2412)"))) %>%
            dplyr::mutate(stovecat = lvls_reorder(stovecat, c(1,2,3,4,5)))  # set factor level order for plots
```

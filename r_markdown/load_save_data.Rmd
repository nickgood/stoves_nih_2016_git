...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  samples <- dplyr::mutate(samples, stove =
                           forcats::lvls_revalue(stove,
                           c("Three Stone Fire (Artisan)",
                             "Clay Ring (Artisan)",
...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  samples <- dplyr::mutate(samples, fuel = 
                           forcats::lvls_revalue(fuel,
                                         c("Douglas Fir",
                                           "Oak",
                                           "Eucalyptus",
                                           "Lump Hardwood (Sm.)",
...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  samples <- dplyr::mutate(samples, stove_fuel = 
                           forcats::as_factor(paste(stove, ":", fuel))) %>%
             dplyr::mutate(stove_fuel = 
                           forcats::lvls_reorder(stove_fuel,
                                        c(28,30,29,7,9,8,1,3,2,25,27,
                                          26,10,12,11,6,5,4,21,20,19,
                                          18,17,14,13,16,15,31,23,24,22)))  # set logical factor order
```
...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  samples <- dplyr::mutate(samples,
                           fuelcat = forcats::lvls_reorder(fuelcat, c(1,2,3,4,5))) %>%  # set factor level order for plots
             dplyr::mutate(stovecat =
                           forcats::fct_collapse(stove,
                             ...
                             advanced = c("Pressurized LPG (WokSmith, WS-C1-25K)",
                                          "Wick Kerosene (Cook 'n Lite, 82)", 
                                          "Pressurized Kerosene (Butterfly, 2412)"))) %>%
            dplyr::mutate(stovecat = forcats::lvls_reorder(stovecat, c(1,2,3,4,5)))  # set factor level order for plots
```

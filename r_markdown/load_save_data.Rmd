...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  samples <- dplyr::mutate(samples,
                           stove_fuel_combo = 
                           as.factor(paste(stove, ":", fuel))) %>%
             dplyr::mutate(stove_fuel = 
                           factor(stove_fuel,
                           levels = 
                           c("Three Stone Fire (Artisan) : Douglas Fir",
...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  samples <- dplyr::mutate(samples,
                           fuelcat = factor(fuelcat,
                                            levels(fuelcat)[c(1,2,3,4,5)])) %>%  # set factor level order for plots
             dplyr::mutate(stovecat =
                           fct_collapse(stove,
                             ...
                             advanced = c("Pressurized LPG (WokSmith, WS-C1-25K)",
                                          "Wick Kerosene (Cook 'n Lite, 82)", 
                                          "Pressurized Kerosene (Butterfly, 2412)"))) %>%
            dplyr::mutate(stovecat = factor(stovecat,levels(stovecat)[c(1,2,3,4,5)]))  # set factor level order for plots
```

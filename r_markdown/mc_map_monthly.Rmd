...
```{r}
  mod_pm <- lm(1000*ef ~ mc_dry, dplyr::filter(df_ef, pol=="grav"))
  summary(mod_pm)
```
...
```{r}
  lmtest::bptest(mod_pm)
```

Check that the residuals are normally distributed.

```{r}
  shapiro.test(mod_pm$residuals)
```
...
```{r}
  mod_co <- lm(1000*ef ~ mc_dry, dplyr::filter(df_ef, pol=="co"))
  summary(mod_co)
```
...
```{r}
  lmtest::bptest(mod_co)
```

Check that the residuals are normally distributed.

```{r}
  shapiro.test(mod_co$residuals)
```
...
```{r}
  lmtest::bptest(mod_ch2o)
```

Check that the residuals are normally distributed.

```{r}
  shapiro.test(mod_ch2o$residuals)
```
...
```{r}
  lmtest::bptest(mod_bz)
```

Check that the residuals are normally distributed.

```{r}
  shapiro.test(mod_bz$residuals)
```

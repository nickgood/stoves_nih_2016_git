...
```{r}
  if(file.exists("../r_files/df_temp_rh_201706.RData")){ # If the averaged data already exists,
    load("../r_files/df_temp_rh_201706.RData")           # load it.          
  }else{                                      # Otherwise,...
    ...
    save(df, file="../r_files/df_temp_rh_201706.RData")
  }
```
...
```{r}
  mod_pm <- lm(log10(1000*ef) ~ mc_dry, dplyr::filter(df_ef, pol=="grav"))
  summary(mod_pm)
```
...
```{r}
  df <- mutate(df, pm_ef = 10^(mod_pm$coefficients[1] + (mod_pm$coefficients[2]*EMC)))
```
...
```{r}
  ...
  scale_ef <- scale_fill_gradientn(colours=palette_ef(100), limits=c(235,800), name=title_ef)
```
...
```{r, fig.width=6.5, fig.height=3.25, fig.align = 'center'}
  ...
  ggsave('../figures/map_pm_ef.pdf', plot=last_plot(), width=16.5/2.54, height=8.25/2.54) # inches
```
...
```{r}
  mod_ch2o <- lm(1000*ef ~ mc_dry, dplyr::filter(df_ef, pol=="formaldehyde"))
  summary(mod_ch2o)
```
...
```{r}
  mod_bz <- lm(1000*ef ~ mc_dry, dplyr::filter(df_ef, pol=="benzene"))
  summary(mod_bz)
```
...
* The ambient temperature in New Dehli, India is `r round(df_dehli$temp, digits=1)` Celsius.  
* The ambient RH in New Dehli, India is `r round(df_dehli$rh, digits=2)`.  
...
* The ambient temperature in Lagos, Nigeria is `r round(df_lagos$temp, digits=1)` Celsius.  
* The ambient RH in Lagos, Nigeria is `r round(df_lagos$rh, digits=2)`.  
...
* The ambient temperature in Khartoum, Sudan is `r round(df_khart$temp, digits=1)` Celsius.  
* The ambient RH in Khartoum, Sudan is `r round(df_khart$rh, digits=2)`.  

...
```{r}
  mod_pm <- lm(1000*g_pol_mjd ~ mc_dry, 
               dplyr::filter(df_ef, pol=="grav", !is.na(mc_dry)))
  summary(mod_pm)
```
...
```{r}
  mod_co <- lm(1000*g_pol_mjd ~ mc_dry, 
               dplyr::filter(df_ef, pol=="co", !is.na(mc_dry)))
  summary(mod_co)
```
...
```{r}
  mod_ch2o <- lm(1000*g_pol_mjd ~ mc_dry, 
                 dplyr::filter(df_ef, pol=="formaldehyde", !is.na(mc_dry)))
  summary(mod_ch2o)
```
...
```{r}
  mod_bz <- lm(1000*g_pol_mjd ~ mc_dry, 
               dplyr::filter(df_ef, pol=="benzene", !is.na(mc_dry)))
  summary(mod_bz)
```

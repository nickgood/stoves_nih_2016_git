...
```{r}
  ...
  if(!file.exists("../r_files/df_temp_rh_2017_weekly.RDS")){ 
    ...
    if(!file.exists("../r_files/df_pm_2017_weekly.RDS")){ 
      ...
      df <- readRDS("../r_files/df_pm_2017_weekly.RDS")
    }
  }
```
...
```{r}
  ...
  df_lon_lat <- dplyr::filter(df, week == 1) %>%
...
Then, add the country names to the data frame of moisture contents.  

```{r}
  df_lon_lat$country <- df_countries$ADMIN 

  df <- left_join(df, df_lon_lat, by=c("lon","lat")) %>%
...
Create and save a plot for each day of the year.

```{r, fig.width=6.2, fig.height=3.25, fig.align = 'center', warning=FALSE}
  ...
  if(!file.exists("../figures/maps/pm52.png")){ 
    ...
    for (i in 1:52){ # for each day of the year
      ...
      # select data to plot
      df_filt <- dplyr::filter(df, week == i)
      ...
      ggsave(fname, plot=last_plot(), width=8.5, height=4.45) # inches
    }
  }
```

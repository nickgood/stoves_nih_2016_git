...
```{r}
  ...
  mce_summary <- mce_summary_by_rep %>%
                 ...
                 group_by(fuel) %>%
                 summarise_all(mean) %>%
                 mutate_at(c("mean", "median", "q_05", "q_95"), .funs = funs(round(. * 100, 1))) %>%
                 ungroup()
  
  mce_summary_per_test <- mce_summary_by_rep %>%
                 select( -rep) %>%
                 group_by(fuel, id) %>%
                 summarise_all(mean) %>%
                 mutate_at(c("mean", "median", "q_05", "q_95"), .funs = funs(round(. * 100, 1))) %>%
...
```{r}
  ...
  saveRDS(mce_summary, file = "../r_files/mce_summary.RDS")
  saveRDS(mce_summary_per_test, file = "../r_files/mce_summary_per_test.RDS")
```

...
```{r, echo=FALSE, fig.height = 8}
 blanks %>%
    ...
    dplyr::mutate(z_score = (val - mean(val))/ sd(val),
    ...
    dplyr::mutate(filter_type = 
                    ...
                    forcats::fct_recode("back puf" = "b",
                                        "front puf" = "f",
                                        "filter" = "ba")) %>%
    ggplot(aes_string(x = "filter_type", y = "z_score")) +
...
## Plot 5: Boxplots of all data by pollutant and test type
...
```{r, echo=FALSE, fig.height = 20}
  pah %>%
    ...
    ggplot(aes_string(y = "val_lod_adj", x = "test_type", fill = "test_type")) +
      ...
      coord_cartesian(ylim = quantile(val_lod_adj, c(0.1, 0.9))) +
      theme_bw() +
      ylab("") +
      xlab("") + 
      theme(text = element_text(size = 20),
            legend.position = "none") +
      facet_grid(pol~filter_type, scales = "free")
```
...
## Plot 7: PAH concentrations by stove type
...
## Plot 8: PAH concentrations by fuel type
...
## Table 5: Burn samples below lod for chemical analysis
...
## Table 6: Burn samples with NA values

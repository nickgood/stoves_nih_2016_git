...
We are missing data for the following tests:

```{r}
  setdiff(filter(select(samples, id), !grepl("^G", id)), select(pah_samples, id)) %>%
    knitr::kable("markdown", align = 'c')
```
...
## Plot 8: Boxplots of all data by pollutant and filter type
...
```{r, echo=FALSE, fig.height = 20}
  pah %>%
    ...
    ggplot(aes_string(y = "val_lod_adj", x = "filter_type", fill = "filter_type")) +
      ...
      theme(text = element_text(size = 20),
            legend.position = "none") +
      facet_wrap(pol~test_type, ncol = 2, scales = "free")
```

...
```{r}
  ...
  dplyr::do(val = round(nrow(filter(., lod == "below"))/nrow(.) * 100, 0)) %>%
  ...
  knitr::kable("markdown", align = 'c',
               caption = "Percent of lab blanks below lod for chemical analysis (%)")
```
...
```{r}
  ...
  dplyr::mutate(filter_type = 
                  ...
                  forcats::fct_recode("back puf" = "b",
                                      ...
                                      "filter" = "ba")) %>%
  dplyr::group_by(filter_type, pol) %>%
  dplyr::do(val = round(nrow(filter(., is.na(val_lod_adj) == TRUE))/nrow(.) * 100, 0)) %>%
  dplyr::mutate(val = cell_spec(val, "html", color = ifelse(val > 10, "red", "blue"))) %>%
  tidyr::spread(filter_type, val) %>%
  dplyr::rename("pah type" = "pol") %>%
  knitr::kable("markdown", align = 'c',
               caption = "Percent of lab blanks with NA values (%)")
```
...
```{r}
  ...
  dplyr::rename("pah type" = "pol") %>%
  knitr::kable("markdown", align = 'c',
               caption = "Percent of backgrounds below the lod of chemical analysis (%)")
```
...
```{r}
  ...
  dplyr::rename("pah type" = "pol") %>%
  knitr::kable("markdown", align = 'c',
               caption = "Percent of backgrounds with NA values (%)")
```
...
```{r}
  ...
  dplyr::mutate(filter_type = 
                  ...
                  forcats::fct_recode("back puf" = "b",
                                      ...
                                      "filter" = "p")) %>%
  dplyr::group_by(filter_type, pol) %>%
  dplyr::do(val = round(nrow(filter(., bk_lod == "below"))/nrow(.), 0) * 100) %>%
  dplyr::mutate(val = cell_spec(val, "html", color = ifelse(val > 10, "red", "blue"))) %>%
  tidyr::spread(filter_type, val) %>%
  dplyr::rename("pah type" = "pol") %>%
  knitr::kable("markdown", align = 'c',
               caption = "Percent of backgrounds below lab blank lod (%)")
```
...
```{r}
  ...
  dplyr::mutate(filter_type = 
                  ...
                  forcats::fct_recode("back puf" = "b",
                                      ...
                                      "filter" = "ba")) %>%
  dplyr::group_by(filter_type, pol) %>%
  dplyr::do(val = round(nrow(filter(., bk_loq == "below"))/nrow(.) * 100, 0)) %>%
  dplyr::mutate(val = cell_spec(val, "html", color = ifelse(val > 10, "red", "blue"))) %>%
  tidyr::spread(filter_type, val) %>%
  dplyr::rename("pah type" = "pol") %>%
  knitr::kable("markdown", align = 'c',
               caption = "Percent of backgrounds below lab blank loq (%)")
```
...
```{r}
  ...
  dplyr::rename("pah type" = "pol") %>%
  knitr::kable("markdown", align = 'c',
               caption = "Percent of burn samples below the lod of chemical analysis (%)")
```

* Total percent of samples below lod for chemical analysis: `r round(nrow(filter(pah_merged, lod == "below")) / nrow(pah_merged), 2) * 100`.
...
```{r}
  ...
  dplyr::rename("pah type" = "pol") %>%
  knitr::kable("markdown", align = 'c',
               caption = "Percent of burn samples with NA values (%)")
```

* Total percent of samples with NA values: `r round(sum(is.na(pah_merged$conc)) / nrow(pah_merged), 2) * 100`.
...
```{r}
  ...
  dplyr::mutate(filter_type = 
                  ...
                  forcats::fct_recode("back puf" = "b",
                                      ...
                                      "filter" = "p")) %>%
  dplyr::group_by(filter_type, pol) %>%
  dplyr::do(val = round(nrow(filter(., conc <= 0))/nrow(.) * 100, 0)) %>%
  dplyr::mutate(val = cell_spec(val, "html", color = ifelse(val > 10, "red", "blue"))) %>%
  tidyr::spread(filter_type, val) %>%
  dplyr::rename("pah type" = "pol") %>%
  knitr::kable("markdown", align = 'c',
               caption = "Percent of burn samples below the background (%)")
```

* Total percent of samples that are negative after background correction: `r round(sum(pah_merged$conc <= 0, na.rm = TRUE) / nrow(pah_merged), 3) * 100`.
...
```{r}
  ...
  dplyr::mutate(filter_type = 
                  ...
                  forcats::fct_recode("back puf" = "b",
                                      ...
                                      "filter" = "p")) %>%
  dplyr::group_by(filter_type, pol) %>%
  dplyr::do(val = round(nrow(filter(., bk_lod == "below"))/nrow(.) * 100, 0)) %>%
  dplyr::mutate(val = cell_spec(val, "html", color = ifelse(val > 10, "red", "blue"))) %>%
  tidyr::spread(filter_type, val) %>%
  dplyr::rename("pah type" = "pol") %>%
  knitr::kable("markdown", align = 'c',
               caption = "Percent of burn samples below the lab blank lod (%)")
```

* Total percent of samples below blank lod: `r round(nrow(filter(pah_merged, bk_lod == "below")) / nrow(pah_merged), 2) * 100`.
...
```{r}
  ...
  dplyr::rename("pah type" = "pol") %>%
  knitr::kable("markdown", align = 'c',
               caption = "Percent of burn samples below lab blank loq (%)")
```

* Total percent of samples below blank loq: `r round(nrow(filter(pah_merged, bk_loq == "below")) / nrow(pah_merged), 2) * 100`.
* Total percent of 'bad' sample values: `r 1 - round(nrow(filter(pah_merged, lod != "below", bk_lod != "below", bk_loq != "below", !is.na(val_bk_adj), pah_merged$conc > 0)) / nrow(pah_merged), 2) * 100`.

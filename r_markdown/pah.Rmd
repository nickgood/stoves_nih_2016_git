...
```{r, echo=FALSE, fig.height = 60}
  pah_samples %>%
    ...
    dplyr::mutate(fraction = (bg_mean / conc) * 100)  %>%
    ...
    dplyr::mutate(stove = 
                  ...
                  fuel = 
                    fuel %>% 
                    tolower) %>%
    ggplot(aes_string(x = "stove", y = "fraction")) +
      geom_jitter(aes(shape = factor(filter_type)), size = 4,  width = 0.2) + 
      scale_shape(solid = FALSE) +
      guides(fill = FALSE) +
      theme_bw() +
      scale_y_log10() +

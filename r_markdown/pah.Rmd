...
```{r}
  ...
  dplyr::do(val = round(nrow(filter(., lod == "below"))/nrow(.) * 100, 2)) %>%
  ...
  knitr::kable("markdown", align = 'c',
               caption = "Percent of blanks below lod for chemical analysis")
```
...
```{r}
  ...
  dplyr::mutate(filter_type = 
                  ...
                  forcats::fct_recode("back puf" = "b",
                                      ...
                                      "filter" = "ba")) %>%
  dplyr::group_by(filter_type, pol) %>%
  dplyr::do(val = nrow(filter(., is.na(val_lod_adj) == TRUE))/nrow(.) * 100) %>%
  dplyr::mutate(val = cell_spec(val, color = ifelse(val > 10, "red", "blue"))) %>%
  knitr::kable(digits = 2, align = 'c',
               caption = "Summary of lab blanks with NA values",
               col.names = c("type of pah", "filter type", "percent with NA (%)"))
```

* Total percent of blanks with NA measurements: `r round(sum(is.na(blanks$val)) / nrow(blanks), 2) * 100`.
* Total percent of 'bad' blank measurements: `r round((nrow(filter(blanks, lod == "below")) + sum(is.na(blanks$val))) / nrow(blanks), 2) * 100`.
...
Calculate new value if measurement below cookstove lab lod or loq:
...
```{r}
  ...
  dplyr::mutate(filter_type = 
                  ...
                  forcats::fct_recode("back puf" = "b",
                                      ...
                                      "filter" = "ba")) %>%
  dplyr::group_by(filter_type, pol) %>%
  dplyr::do(val = nrow(filter(., lod == "below"))/nrow(.) * 100) %>%
  dplyr::mutate(val = cell_spec(val, color = ifelse(val > 10, "red", "blue"))) %>%
  knitr::kable(digits = 2, align = 'c',
               caption = "Summary of lab backgrounds below chemical analysis lod",
               col.names = c("type of pah", "filter type", "percent below lod (%)"))
```
...
```{r}
  ...
  dplyr::mutate(val = cell_spec(val, color = ifelse(val > 10, "red", "blue"))) %>%
  knitr::kable(digits = 2, align = 'c',
               caption = "Summary of lab backgrounds with NA",
               col.names = c("type of pah", "filter type", "percent with NA (%)"))
```

* Total percent of backgrounds with NA values: `r round(sum(is.na(bg$val)) / nrow(bg), 2) * 100`.
...
```{r}
  ...
  knitr::kable(digits = 1, align = 'c',
               caption = "Summary of lab backgrounds below lab blank lod")
```
...
```{r}
  ...
  dplyr::mutate(filter_type = 
                  ...
                  forcats::fct_recode("back puf" = "b",
                                      ...
                                      "filter" = "ba")) %>%
  dplyr::group_by(filter_type, pol) %>%
  dplyr::do(val = nrow(filter(., bk_loq == "below"))/nrow(.) * 100) %>%
  dplyr::mutate(val = cell_spec(val, color = ifelse(val > 10, "red", "blue"))) %>%
  knitr::kable(digits = 2, align = 'c',
               caption = "Summary of lab backgrounds below lab blank loq",
               col.names = c("type of pah", "filter type", "percent below loq (%)"))
```

* Total percent of backgrounds below blank loq: `r round(nrow(filter(bg, bk_loq == "below")) / nrow(bg), 2) * 100`.
* Total percent of 'bad' background values: `r 1 - round(nrow(filter(bg, lod != "below", bk_lod != "below", bk_loq != "below", !is.na(val_bk_adj))) / nrow(bg), 2) * 100`.
...
* Fraction of samples below lod for chemical analysis: `r round(nrow(filter(pah_merged, lod == "below")) / nrow(pah_merged), 2) * 100`.
...
* Fraction of samples with NA values: `r round(sum(is.na(pah_merged$conc)) / nrow(pah_merged), 2) * 100`.
...
```{r}
  ...
  dplyr::mutate(filter_type = 
                  ...
                  forcats::fct_recode("back puf" = "b",
                                      ...
                                      "filter" = "ba")) %>%
  dplyr::group_by(filter_type, pol) %>%
  dplyr::do(val = nrow(filter(., conc <= 0))/nrow(.) * 100) %>%
  dplyr::mutate(val = cell_spec(val, color = ifelse(val > 10, "red", "blue"))) %>%
  knitr::kable(digits = 2, align = 'c',
               caption = "Summary of samples that are negative after background correction",
               col.names = c("type of pah", "filter type", "percent with NA (%)"))
```

* Fraction of samples that are negative after background correction: `r round(sum(pah_merged$conc <= 0, na.rm = TRUE) / nrow(pah_merged), 3) * 100`.
...
```{r}
  ...
  dplyr::mutate(filter_type = 
                  ...
                  forcats::fct_recode("back puf" = "b",
                                      ...
                                      "filter" = "ba")) %>%
  dplyr::group_by(filter_type, pol) %>%
  dplyr::do(val = nrow(filter(., bk_lod == "below"))/nrow(.) * 100) %>%
  dplyr::mutate(val = cell_spec(val, color = ifelse(val > 10, "red", "blue"))) %>%
  knitr::kable(digits = 2, align = 'c',
               caption = "Summary of burn samples below lab blank lod",
               col.names = c("type of pah", "filter type", "percent below loq (%)"))
```

* Fraction of samples below blank lod: `r round(nrow(filter(pah_merged, bk_lod == "below")) / nrow(pah_merged), 2) * 100`.
...
```{r}
  ...
  dplyr::mutate(val = cell_spec(val, color = ifelse(val > 10, "red", "blue"))) %>%
  knitr::kable(digits = 2, align = 'c',
               caption = "Summary of burn samples below lab blank loq",
               col.names = c("type of pah", "filter type", "percent below loq (%)"))
```

* Fraction of samples below blank loq: `r round(nrow(filter(pah_merged, bk_loq == "below")) / nrow(pah_merged), 2) * 100`.
* Fraction of 'bad' sample values: `r 1 - round(nrow(filter(pah_merged, lod != "below", bk_lod != "below", bk_loq != "below", !is.na(val_bk_adj), pah_merged$conc > 0)) / nrow(pah_merged), 2) * 100`.
...
Histogram of blank test and background data

* mass on filter given on x-axis in ug

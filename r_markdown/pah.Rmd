...
```{r load_data}
  ...
  pah_lod_1 <- readRDS("../r_files/pah_lod_1.RDS")    # pah lod_1 dataset
```
...
## Merge raw and lod_1 data

Extract chemical analysis lod_1 data

```{r}
  lod_1 <-
    pah_lod_1 %>%
    dplyr::mutate(lod_1 = ifelse(val == "BDL", "below", "above")) %>%
    dplyr::select(id, lod_1, filter_type, pol)
```

Calculate new value if measurement below lod_1

```{r}
  pah <- 
    pah_raw %>% 
    dplyr::left_join(lod_1, by = c("id", "filter_type", "pol")) %>%
    dplyr::mutate(val = as.numeric(val),
                  val_lod_1_adj = ifelse(lod_1 == "below", val / sqrt(2), val))
```
...
* color indicates if sample was above or below instrument lod (lod_1)
 ...
 blanks %>%
    ...
    dplyr::mutate(filter_type = 
                    ...
                    forcats::fct_recode("puf" = "b",
                                        "puf" = "f",
                                        "filter" = "ba")) %>%
    ggplot(aes_string(x = "filter_type", y = "val_lod_1_adj", colour = "lod_1")) +
...
Fraction of blanks below lod_1: `r round(nrow(filter(blanks, lod_1 == "below")) / nrow(blanks), 2)`

Fraction of blanks with NA values: `r round(sum(is.na(blanks$val)) / nrow(blanks), 2)`

Fraction of blanks either below lod_1 or NA value: `r round((nrow(filter(blanks, lod_1 == "below")) + sum(is.na(blanks$val))) / nrow(blanks), 2)`
...
```{r}
  signal <-
    ...
    dplyr::summarise(mean = mean(val_lod_1_adj, na.rm = TRUE),
                     lod_2 = 3 * sd(val_lod_1_adj, na.rm = TRUE),
                     loq_2 = 10 * sd(val_lod_1_adj, na.rm = TRUE))
```
...
```{r}
  pah_merged <-
    ...
    dplyr::mutate(dur = (end - start) / 60,
                  conc = val_lod_1_adj * 1000 / (flow * dur))
```
...
Find which variables are below blank lod (lod_2) and loq
...
* color indicates if sample was above or below lod_1 calculated from lab blanks
* mean shown with an x
* median shown with a square
  ...
  bg %>%
    ...
    dplyr::mutate(filter_type = 
                    ...
                    forcats::fct_recode("back puf" = "b",
                                        "front puf" = "f",
                                        "filter" = "p")) %>%
    ggplot(aes_string(x = "filter_type", y = "conc", colour = "lod_1")) +
      geom_point(size = 3, shape = 1, stroke = 1) +
      geom_point(stat = "summary", fun.y = "mean", colour = "black",
                 shape = 4, size = 4, stroke = 2) +
      geom_point(stat = "summary", fun.y = "median", colour = "black",
...
Fraction of backgrounds below lod_1 for chemical analysis

Fraction of backgrounds with NA values

Fraction of backgrounds below lod_1 and LOQ calculated from lab blanks
...
Fraction of samples below lod_1 for chemical analysis

Fraction of samples below lod_1 and LOQ calculated from lab blanks

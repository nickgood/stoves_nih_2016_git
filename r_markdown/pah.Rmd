...
```{r}
  pah_merged <-
    ...
    dplyr::mutate(val_bk_adj = val_lod_adj - bk_mean,
                  ...
                  val_bk_adj = ifelse(bk_lod == "below" & bk_lod == "below", (3 * bk_sd) / sqrt(2), val_bk_adj),
                  val_bk_adj = ifelse(bk_lod == "above" & bk_loq == "below", (10 * bk_sd) / sqrt(2), val_bk_adj))
```
...
Fraction of backgrounds either below lod for chemical analysis or NA value: `r round((nrow(filter(bg, lod == "below")) + sum(is.na(bg$val))) / nrow(bg), 2)`.
...
Plot all sample concentrations in ug per meter cubed:
...
Plot all sample concentrations (grouped by stove type and colored by fuel type) in ug per meter cubed:
...
```{r, echo=FALSE, fig.height = 15}
  pah_merged %>%
    dplyr::filter(!is.na(conc)) %>%
    dplyr::left_join(samples, stove, fuel, by = "id") %>%
    dplyr::mutate(fuel =
                    fuel %>% 
                    factor %>%
                    tolower %>% 
                  ...
                  stove = 
                    ...
                    forcats::fct_relevel("three \n stone \n fire",
                                         ...
                                         "pressure \n kerosene",
                                         "lpg")) %>%
    ggplot(aes_string(x = "filter_type", y = "conc", colour = "fuel")) +
      geom_point(size = 3, shape = 1, stroke = 1) +
      geom_point(stat = "summary", fun.y = "mean", colour = "black",
                 shape = 4, size = 4, stroke = 2) +
      geom_point(stat = "summary", fun.y = "median", colour = "black",

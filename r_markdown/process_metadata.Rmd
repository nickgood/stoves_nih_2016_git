...
I modified this section of code on Dec. 7, 2018

```{r}

  # select "m_dry" and variables needed to match with fuel data
  mass_dry_new <- dplyr::select(fuel_clean, fuel_id, test_order, mass_original, m_dry) %>%
                  plyr::rename(c("fuel_id"       = "id",
                                 "test_order"    = "order",
                                 "m_dry"         = "mass_dry_new",
                                 "mass_original" = "mass_start"))

  # Add "mass_dry_new" to the fuel data frame
  fuel_mass <- dplyr::left_join(fuel, mass_dry_new, by=c("id","order","mass_start")) %>%
               dplyr::select(-mass_wet, -date_wet_start, -time_wet_start, 
                             -date_wet_end, -time_wet_end, -dur_wt, -notes)          %>%
    
               # Removed FH2 rep 3 because the fuel was not burned
               dplyr::filter((id=="FH2" & order==3) == FALSE)                        %>%
    
               # Select only mass data and sum masses over the test
               dplyr::select(id, order, starts_with("mass"))                         %>%
               dplyr::group_by(id)                                                   %>%
               dplyr::summarise_at(vars(starts_with("mass")), funs(sum, first))      %>%
               dplyr::select(1:8) %>%
               tidyr::gather("var", "val", 2:8)
```

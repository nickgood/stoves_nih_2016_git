...
```{r load_files}
  ...
  fuel_clean <- readRDS("../r_files/fuel_prep_clean.RDS")
```
...
```{r}
  samples <-   test_log %>%
               ...
               dplyr::left_join(dplyr::select(fuel, id, mc_target) %>%
                                dplyr::distinct(),
                                by = "id") %>%
...
```{r}
 # remove FH2 rep 3 becuase fuel was not burned
  fuel_no_fh2_3 <- dplyr::setdiff(fuel,
                                  dplyr::filter(fuel, (id == "FH2" & order == "3")))

  fuel_clean_no_fh2_3 <- dplyr::setdiff(fuel_clean,
                                  dplyr::filter(fuel_clean, (fuel_id == "FH2" & test_order == "3"))) %>%
                         dplyr::rename(id = fuel_id, mass_dry_new = m_dry) %>%
                         dplyr::select(id, mass_dry_new)
  
  fuel_clean_no_fh2_3 <- cbind(fuel_clean_no_fh2_3, fuel_no_fh2_3$order)
  fuel_clean_no_fh2_3 <- rename(fuel_clean_no_fh2_3, order = `fuel_no_fh2_3$order`) %>%
                         select(mass_dry_new)
  
  fuel_clean_no_fh2_3 <- cbind(fuel_no_fh2_3, fuel_clean_no_fh2_3)
  fuel_clean_no_fh2_3 <- select(fuel_clean_no_fh2_3, -13, -14, -15, -16, -17, -18, -19)
    
  fuel_mass <- dplyr::select(fuel_clean_no_fh2_3, id, order, starts_with("mass")) %>%
               dplyr::group_by(id) %>%
               dplyr::summarise_at(vars(starts_with("mass")), funs(sum, first)) %>%
               tidyr::gather("var", "val", 2:15)

  pot_mass <- dplyr::select(test_log, id, starts_with("wgt")) %>%
              dplyr::rename(wgt_water_pot = wgt_pot_a,
                            wgt_water_on = wgt_on_1,
                            wgt_water_off = wgt_off_1) %>%
               tidyr::gather("var", "val", 2:26) %>%
               dplyr::filter(!is.na(val), id != "BG2")

  test_mass <- dplyr::bind_rows(fuel_mass, pot_mass) %>%
               dplyr::select(id, var, val)

  saveRDS(test_mass, file="../r_files/fuel_wgts.RDS")
```

```{r, echo=FALSE, fig.height=8}
  p_data <- dplyr::left_join(fuel_mass, samples, by = "id") %>%

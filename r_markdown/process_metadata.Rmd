...
# Add test type to test log

```{r}
  test_log <- test_log %>%
              dplyr::mutate(type = ifelse(grepl("^L", id), "lab", "field")) %>%
              dplyr::mutate(type = ifelse(grepl("^B", id), "bg", type)) %>%
              dplyr::mutate(fp_target = ifelse(grepl("^LL|^FL", id), "low", "med")) %>%
              dplyr::mutate(fp_target = ifelse(grepl("^LH|^FH", id), "high", fp_target)) %>%
              dplyr::mutate(fp_target = ifelse(grepl("^B", id), NA, fp_target))
```
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, results='hide', cache=FALSE, fig.width=20, fig.height=12}
  test_list <- test_log %>%
               dplyr::select(id, stove, fuel, type, fp_target) %>%
               dplyr::left_join(dplyr::select(fuel, fuel_id, mc_target), by = c("id" = "fuel_id")) %>%
               dplyr::distinct() %>%
               dplyr::group_by(mc_target, fp_target, type) %>%
               dplyr::summarise(id = paste(id, collapse = ","))

  ggplot(test_list, aes(y = mc_target, x=fp_target)) + 

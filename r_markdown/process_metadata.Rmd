...
```{r}
  samples <-   test_log %>%
               dplyr::select(id, date, stove, fuel, type, mc_target) %>%
...
```{r}
  fuel_mass <- dplyr::filter(tidy_id_date(fuel, "^mass_", "mass_"), !is.na(value))

  pot_mass <- dplyr::filter(tidy_id_date(test_log, "^wgt_", "wgt_"), !is.na(value))
...
```{r, echo=FALSE}
  p_data <- dplyr::group_by(fuel_mass, var) %>%
...
`r (as.numeric(count_samples[2,2])` field wood tests, `r (as.numeric(count_samples[2,3])` lab wood tests, and `r (as.numeric(count_samples[2,2])` background tests were conducted for a total of `r sum(count_samples$n)` tests. The tests were performed between `r min(samples$date)` and `r max(samples$date)`. The `r samples$stove[1]` stove was tested with `r samples$fuel[1]` fuel.

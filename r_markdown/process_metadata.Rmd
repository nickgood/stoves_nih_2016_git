...
```{r}
  samples <-   test_log %>%
               dplyr::select(id, date, stove, fuel, type, mc_level) %>%
...
p_box <- ggplot(p_data, aes(x = var, y = value_norm)) +
           geom_boxplot() +
           geom_text(aes(label = outlier), na.rm = TRUE, hjust = 0.2, size = 4) +
           theme_bw(base_size = 14) +
  ...
  p_hist <- ggplot(p_data, aes(x = value, fill = mc_target)) +
...
```{r, echo=FALSE}
  ...
  p_hist <-  ggplot(p_data, aes(x = factor(type), y = value_norm)) +
               geom_boxplot() +
               geom_text(aes(label = outlier), na.rm = TRUE, hjust = -0.3, size = 4) +
               theme_bw(base_size = 14) +
               ylab("z score normalized value") +

...
```{r, echo=FALSE, fig.height=10, fig.width=8}
plot_pol_conc3 <- dplyr::filter(plot_pol_conc, metric == "hood_conc")

  ggplot(data = plot_pol_conc3, aes(x = stove, y = value, color = metric)) +
    geom_point(size = 3, shape = 1, stroke = 1.5) +
    theme_bw() + 
    facet_wrap(~pol, scales = "free_y", ncol = 2) +
    scale_y_continuous(breaks = waiver()) +
...
```{r, echo=FALSE}
  grav <- dplyr::filter(plot_pol_conc, pol == "pm2.5") %>%
          tidyr::spread(pol, value) %>%
          dplyr::select(-inst)

  plot_pol_conc4 <- dplyr::filter(plot_pol_conc, pol != "pm2.5") %>%
                    dplyr::left_join(grav, by = c("stove", "metric")) %>%
                    dplyr::filter(metric == "diluted_conc")

```

```{r, echo=FALSE, fig.height=10, fig.width=8}
  ggplot(data = plot_pol_conc4, aes(x = pm2.5, y = value, color = metric)) +
    geom_point(size = 3, shape = 1, stroke = 1.5) +
    theme_bw() + 
    facet_wrap(~pol, scales = "free_y", ncol = 2) +
    scale_y_continuous(breaks = waiver()) +

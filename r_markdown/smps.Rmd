...
I updated the next three chunks of code on Dec. 4, 2018. -- J.T. 
...
```{r, fig.width=3.54, fig.height=3.15, fig.align='center'}
  ...
  ggsave(filename="smps.pdf", plot=last_plot(), device="pdf", width=9/2.54, height=8/2.54)
```
...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  ...
  df_ufp <- dplyr::filter(smps_dilution, size_nm <= 100) %>% # get particles smaller than 100 nm
            ...
            # Add test metadata
            dplyr::left_join(mean_mc, by=c("id")) %>%
            dplyr::left_join(dplyr::select(samples, id, type, mc_level), by=c("id")) %>%
            dplyr::mutate(type = ifelse(type=="lab", "Milled", "Split"))   %>%
            dplyr::filter(!is.na(mean_mc))
```

```{r, fig.width=3.15, fig.height=3.15, fig.align='center'}

  ggplot(data=df_ufp, aes(x=mean_mc, y=num, shape=type)) + 
  ...
  ggsave(filename="ufp.pdf", plot=last_plot(), device="pdf", width=8/2.54, height=8/2.54)
```
...
```{r}
  ...
  #kable(df_ufp_summary, format="markdown", digits=c(2,1),
...
smps_conc1 <- group_by(smps_conc, mc_level, size_nm) %>%
              mutate(conc_avg = mean(conc), conc_sd = sd(conc)) %>%
              ungroup() %>%
...
ggplot(smps_conc1, aes(x = size_nm, y = conc_avg, color = mc_level)) +
  geom_line(size = 2) +
  scale_y_log10() +
  geom_errorbar(aes(ymin = conc_avg-conc_sd, ymax = conc_avg+conc_sd), width = 0.2) +

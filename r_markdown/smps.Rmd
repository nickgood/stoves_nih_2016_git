...
smps_dilution <- select(smps_merged, id, size_nm, dw) %>%
                 ...
                 left_join(dilution, by = "id") %>%
                 mutate(dil_num = avg_val*median_dilution) %>%
                 group_by(mc_level,size_nm) %>%
                 summarise(avg_num = mean(dil_num), sd_num = sd(dil_num))

ggplot(smps_dilution, aes(x = size_nm, y = avg_num, color = mc_level)) +
  geom_line(size = 1) +
  #scale_y_log10() +
  geom_errorbar(aes(ymin = avg_num-sd_num, ymax = avg_num+sd_num), width = 0.2, alpha =0.5) +
  scale_color_manual(values = c("darkorange", "darkorchid3", "blue")) +
  theme_bw() +
  labs(color = "MC Level", x = "Particle Size [nm]", y = "dN/dlog(Dp)")
```

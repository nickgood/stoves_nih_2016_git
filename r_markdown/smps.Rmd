...
* calculate the sub-100 nm number concentration for each smps scan 
...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  smps_ultrafine_bg <- dplyr::filter(smps_merged, type == "bg") %>%
                         dplyr::select(datetime, date, time, id, sample,
                                       value, size, qc) %>%
                         dplyr::filter(size <= 100) %>%

...
```{r fig.width = 16, fig.height = 11}
  p_data_key <- dplyr::filter(per_test, grepl('^grav$|^co$|^formaldehyde$|^acetaldehyde$|^voc_benzene$|^voc_toluene$', pol)) %>% #add ECOC when have data
                ...
                dplyr::mutate(pol = forcats::fct_relevel(pol, "PM2.5", "co",
                                                      ...
                                                      "benzene", "toluene")) %>%
                dplyr::mutate(fuel_names = forcats::fct_relevel(fuel_names, "wood shims", "kindling", 
                                                                       "newspaper", "fabric", 
...
* variation in emissions per test for key pollutants - as a percentage

```{r}
var_per_fuel_pct <- mean_per_startup %>%
                            dplyr::filter(grepl('^grav$|^co$|^formaldehyde$|^acetaldehyde$|^voc_benzene$|^voc_toluene$', pol)) %>% #add ECOC when have data)
...
* variation in emissions per test for key pollutants - as g/startup actual value
...
palette <- c("#E69F00", "#009E73", "#993333", "#0072B2", "#999999", "#D55E00")
  ...
  ggplot(pol_cats, aes(fuel_names, mean)) +
        ...
        theme_bw() +
        theme(legend.position = "bottom",text = element_text(size=25), axis.text.x = element_text(angle = 45, hjust = 1)) +
        scale_fill_manual(values = palette) +
        theme(legend.title=element_blank(),
              axis.text.y = element_text(margin = margin(t = 0, r = 0, b = 0, l = 25))) +
        ylab("proportion of category total") + 
...
```{r fig.width = 7}
  pmco <- dplyr::filter(per_test, grepl('^grav$|^co$', pol)) %>%
          ...
          dplyr::select(id, fuel_names, fuel_cat, pol, mass_pol_per_startup_std) %>%
          tidyr::spread(pol, mass_pol_per_startup_std)

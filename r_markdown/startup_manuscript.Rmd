...
```{r, echo = FALSE, fig.width = 12, fig.height = 17}
  ...
  ylab("Emissions Factor (mg per startup event)") + xlab("Fuel Type") +
...
stacked_plot <- ggplot(voc_cats, aes(fuel_names, mean)) +
...
```{r}
  ...
  pm_palette <- c("elemental carbon" = "grey45", "organic carbon" = "seagreen4", "PM2.5 mass" = "grey15")
...
gridExtra::grid.arrange(stacked_plot, pm_plot, ncol = 2)
```
...
```{r, echo = FALSE, fig.width = 9, fig.height = 4}
  ...
  pol_cats2 <- per_fuel_mass %>%
               ...
               dplyr::mutate(pol = sub("ec", "elemental carbon", pol),
                            pol = sub("oc", "organic carbon", pol),
                            pol = factor(pol, levels = c("benzene", "toluene", "ethylbenzene", "xylenes", 

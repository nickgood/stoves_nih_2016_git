...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE, eval=TRUE}
  ...
  regular <- dplyr::filter(temp_merged,
             ...
             dplyr::filter(loc == "flue" | loc == "front")
...
# filter data associated with open circut

```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  #temp_merged <- dplyr::filter(temp_merged, t_oc < 200 | t_oc < - 200 )
```
...
## flue temperature profiles



## temperature profile of front pot

```{r, echo=FALSE, warning=FALSE, message=FALSE, results='markup', cache=FALSE, fig.width=12, fig.height=50}
  p_data <- dplyr::filter(temp_merged, (loc == "front"))

  ggplot(p_data, aes(datetime, t_oc, color = qc)) +
         geom_line() +
         geom_point() +
         scale_colour_manual(values = 
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, results='markup', cache=FALSE, fig.width=12, fig.height=15, fig.show=FALSE}

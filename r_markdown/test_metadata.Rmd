...
Sum total masses of wet and dry fuel.

```{r}
  df_fuel_sum <- dplyr::group_by(df_fuel_prep, id) %>%
                 dplyr::summarise(mass_end  = sum(mass_end),
                                  mass_dry  = sum(mass_dry),
                                  mc_target = mean(mc_target)) %>%
                 dplyr::mutate(mass_h2o = mass_end - mass_dry)

  saveRDS(df_fuel_sum, "../r_files/df_fuel_sum.RDS")
```
...
```{r}
  df_mc <- df_fuel_sum %>%
           dplyr::mutate(mc_dry = 100*(mass_end - mass_dry)/mass_dry) %>%
           dplyr::select(id, mc_target, mc_dry)
```

...
```{r, echo=FALSE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  times <- dplyr::filter(test_times, var == "start_1" | var == "shutdown") %>%
           ...
           dplyr::mutate(dur = (shutdown - start_1) / 60)  # extract test sample durations (minutes)

  times <- dplyr::left_join(dplyr::select(samples, id) %>%
  													dplyr::filter(grepl("^[0-9]", id) == TRUE), 
  													times,
  													by = "id")  # join with sample list
```

```{r, echo=FALSE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  ggplot(times, aes(x = dur)) +
    geom_histogram(binwidth = 15) +
    theme_minimal() +
    xlab("test duration (min)")
```

There are `r sum(is.na(times$dur))` tests with missing sample duration data, these are tests: `r times$id[is.na(times$dur)]`.

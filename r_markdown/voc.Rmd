...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  bg <- dplyr::group_by(bg, pol) %>%
        dplyr::summarise(mean = mean(ppb),
                         ...
                         mean_bg4rm = mean(ppb[id_test != "BG4"]),
                         n = n())
```
...
BG4 test has very high benzene and toluene values. Should probably remove this background test from the VOC data. 

## merge background

```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  bg <- dplyr::select(bg, pol, mean, mean_bg4rm) %>%

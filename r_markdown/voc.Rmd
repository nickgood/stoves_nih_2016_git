...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  voc <- data %>% dplyr::rename(id_kf = voc_kf_id) %>%
...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  bg_mean <- dplyr::select(bg_mean, pol, mean) %>%
             dplyr::rename(ppb_bg = mean)

  voc <- dplyr::left_join(voc, bg_mean, by = "pol") %>%
...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE, fig.width=20, fig.height=25}
  ggplot(voc, aes(id, ppb, colour = fuel)) +
    geom_point(size = 5) +

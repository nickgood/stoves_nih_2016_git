...
load.batch <- function(file, sheet = "Batch Fed Sampling Forms"){
    ...
    # convert to date
      ...
      # relative humidity content (special case)
      ...
      # combine dataframes
        out <- cbind(df_dat, df_num, df_time, df_rh, df_char, df_fac)
...
# df <- load.fivegascal(file)
  load.fivegascal <- function(file, sheet = "Sheet1"){
    ...
    # combine dataframes
      out <- cbind(df_dat, df_num, df_time, df_char)
...
# df <- load.co2_cal_flows_meta(file)
  load.co2_cal_flows_meta <- function(file, sheet = "Sheet1"){
    ...
    # new column names
      ...
      # combine dataframes
        out <- cbind(df_dat, df_num, df_char)
  ...
  load.flow_fivegas_meta <- function(file, sheet = "Tester 2 Data Sheet"){
    ...
    # rename columns
      names(df) <- c("date",
                   ...
                   "time_start_carb",
                   "time_end_carb",
                   "time_start_fivegas_bg_post",
                   "time_end_fivegas_bg_post",
                   "postflow_carb_1",
                   "postflow_carb_2",
                   "postflow_carb_3",
                   "postflow_carb_avg",
